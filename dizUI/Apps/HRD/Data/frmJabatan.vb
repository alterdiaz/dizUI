Public Class frmJabatan
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        If isRestrictedString(teJabatan.Text, dbstring) = True Then
            dizMsgbox("Jabatan tidak boleh " & teJabatan.Text & ", silahkan cek isian anda", dizMsgboxStyle.Peringatan, Me)
            Exit Sub
        End If
        ...
        If statData = statusData.Baru Then
            idData = GenerateGUID()
            field.AddRange(New String() {"idposition", "idcompany", "priority", "position", "positiontype", "isdeleted", "createdby", "createddate"})
            value.AddRange(New Object() {idData, idcomp, sePriority.Value, teJabatan.Text, If(lueType.EditValue, CObj(DBNull.Value)), 0, userid, nowTime})
        Else
            field.AddRange(New String() {"idposition", "priority", "position", "positiontype", "updatedby", "updateddate"})
            value.AddRange(New Object() {idData, sePriority.Value, teJabatan.Text, If(lueType.EditValue, CObj(DBNull.Value)), userid, nowTime})
        End If
        If dtSQL.datasetSave("position", idData, field, value, False) = True Then
            dizMsgbox("Data tersimpan", dizMsgboxStyle.Info, Me)
            isiLog(userid, dbstring, field, value, "position")
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        ...
        If btnDelete.Text = "HAPUS" Then
            field.AddRange(New String() {"idposition", "isdeleted", "updatedby", "updateddate"})
            value.AddRange(New Object() {idData, 1, userid, nowTime})
            retval = sqls.datasetSave("position", idData, field, value, False)
            If retval Then
    ...
    Private Sub gvData_FocusedRowChanged(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs) Handles gvData.FocusedRowChanged
        Try
            ...
            teJabatan.Text = dcol("position")
            lueType.EditValue = dcol("positiontype")
            sePriority.EditValue = dcol("priority")
            idData = dcol("idposition")
    ...
    Private Sub frmJabatan_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        End If
        checkFieldMaxLength(dbstring, tlpField, "position")
        loadLOV()
        btnNew_Click(Me, Nothing)
    End Sub
