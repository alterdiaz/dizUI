...
Public Class frmUnit
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        If teUnit.Text.Replace(" ", "") = "SYSTEM" Then
            dizMsgbox("UNIT tidak boleh SYSTEM, silahkan cek isian anda", dizMsgboxStyle.Peringatan, Me)
            Exit Sub
        End If
        If teUnit.Text.Replace(" ", "") = "VENDOR" Then
            dizMsgbox("UNIT tidak boleh VENDOR, silahkan cek isian anda", dizMsgboxStyle.Peringatan, Me)
            Exit Sub
        End If
        ...
        End If
        If cnt > 0 Then
            dizMsgbox("Kode Unit ini sudah digunakan", dizMsgboxStyle.Peringatan, Me)
            teKode.Focus()
            Exit Sub
        End If
        ...
        If statData = statusData.Baru Then
            field.AddRange(New String() {"idunit", "idcompany", "unit", "kode", "kodeangka", "remarks", "isdeleted", "createdby", "createddate"})
            value.AddRange(New Object() {idData, idcomp, teUnit.Text, teKode.Text, teKodeAngka.Text, teRemarks.Text, 0, userid, nowTime})
        Else
            field.AddRange(New String() {"idunit", "idcompany", "unit", "kode", "kodeangka", "remarks", "updatedby", "updateddate"})
            value.AddRange(New Object() {idData, idcomp, teUnit.Text, teKode.Text, teKodeAngka.Text, teRemarks.Text, userid, nowTime})
        End If

        If dtSQL.datasetSave("unit", idData, field, value, False) = True Then
            dizMsgbox("Data tersimpan", dizMsgboxStyle.Info, Me)

            isiLog(userid, dbstring, field, value, "unit")
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        ...
        If btnDelete.Text = "HAPUS" Then
            field.AddRange(New String() {"idunit", "isdeleted", "updatedby", "updateddate"})
            value.AddRange(New Object() {idData, 1, userid, nowTime})
            retval = sqls.datasetSave("unit", idData, field, value, False)
    ...
    Private Sub frmUnit_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        getRegex(dbstring)
        If formTitle <> "" Then
            Me.Text = formTitle
            lblTitle.Text = formTitle
        End If
        checkFieldMaxLength(dbstring, tlpField, "unit")
    ...
    Private Sub btnPrimaryAlamat_Click(sender As Object, e As EventArgs) Handles btnPrimaryAlamat.Click
        ...
        If btnPrimaryAlamat.Text = "PRIMARY" Then
            Dim sqls As New SQLs(dbstring)
            sqls.DMLQuery("update alamat set isprimary=0 where idreff='" & idData & "' and tablereff='UNIT'", False)
    ...
    Private Sub btnSaveAlamat_Click(sender As Object, e As EventArgs) Handles btnSaveAlamat.Click
        ...
        If statDataChild = statusData.Baru Then
            ...
            If gvAlamat.RowCount = 0 Then
                value.AddRange(New Object() {idDataChild, idcomp, idData, "UNIT", If(lueNegara.EditValue, CObj(DBNull.Value)), If(luePropinsi.EditValue, CObj(DBNull.Value)), If(lueKabupaten.EditValue, CObj(DBNull.Value)), If(lueKecamatan.EditValue, CObj(DBNull.Value)), If(lueKelurahan.EditValue, CObj(DBNull.Value)), meAlamat.Text, lueAddressType.EditValue, teKodepos.Text, 0, 1, userid, nowTime})
            Else
        ...
        ElseIf statDataChild = statusData.Edit Then
            ...
            value.AddRange(New Object() {idDataChild, idData, "UNIT", If(lueNegara.EditValue, CObj(DBNull.Value)), If(luePropinsi.EditValue, CObj(DBNull.Value)), If(lueKabupaten.EditValue, CObj(DBNull.Value)), If(lueKecamatan.EditValue, CObj(DBNull.Value)), If(lueKelurahan.EditValue, CObj(DBNull.Value)), meAlamat.Text, lueAddressType.EditValue, teKodepos.Text, userid, nowTime})
            If dtsql.datasetSave("alamat", idDataChild, field, value, False) = True Then
                isiLog(userid, dbstring, field, value, "alamat")
                dizMsgbox("Alamat telah tersimpan", dizMsgboxStyle.Info, "Informasi", Me)
            End If
    ...
    Private Sub btnSaveEmail_Click(sender As Object, e As EventArgs) Handles btnSaveEmail.Click
        ...
        If statDataChild = statusData.Baru Then
            Dim sqls As New SQLs(dbstring)
            sqls.DMLQuery("select email from email where tablereff='UNIT' and idreff='" & idData & "' and email='" & teEmail.Text & "'", "cek")
            ...
            If gvEmail.RowCount = 0 Then
                value.AddRange(New Object() {idDataChild, idcomp, idData, "UNIT", teEmail.Text, lueEmailType.EditValue, 0, 1, userid, nowTime})
            Else
                value.AddRange(New Object() {idDataChild, idcomp, idData, "UNIT", teEmail.Text, lueEmailType.EditValue, 0, 0, userid, nowTime})
            End If
        ...
        ElseIf statDataChild = statusData.Edit Then
            ...
            value.AddRange(New Object() {idDataChild, idData, "UNIT", teEmail.Text, lueEmailType.EditValue, userid, nowTime})
            If dtsql.datasetSave("email", idDataChild, field, value, False) = True Then
                isiLog(userid, dbstring, field, value, "email")
                dizMsgbox("Email telah tersimpan", dizMsgboxStyle.Info, "Informasi", Me)
            End If
    ...
    Private Sub btnPrimaryEmail_Click(sender As Object, e As EventArgs) Handles btnPrimaryEmail.Click
        ...
        If btnPrimaryAlamat.Text = "PRIMARY" Then
            Dim sqls As New SQLs(dbstring)
            sqls.DMLQuery("update email set isprimary=0 where idreff='" & idData & "' and tablereff='UNIT'", False)
    ...
    Private Sub btnPrimaryTelepon_Click(sender As Object, e As EventArgs) Handles btnPrimaryTelepon.Click
        ...
        If btnPrimaryAlamat.Text = "PRIMARY" Then
            Dim sqls As New SQLs(dbstring)
            sqls.DMLQuery("update phone set isprimary=0 where phonetype='" & luePhoneType.EditValue & "' and idreff='" & idData & "' and tablereff='UNIT'", False)
    ...
    Private Sub btnSaveTelepon_Click(sender As Object, e As EventArgs) Handles btnSaveTelepon.Click
        ...
        If statDataChild = statusData.Baru Then
            ...
            If gvTelepon.RowCount = 0 Then
                value.AddRange(New Object() {idDataChild, idcomp, idData, "UNIT", tePhone.Text, teExtension.Text, luePhoneType.EditValue, 0, 1, userid, nowTime})
            Else
                value.AddRange(New Object() {idDataChild, idcomp, idData, "UNIT", tePhone.Text, teExtension.Text, luePhoneType.EditValue, 0, 0, userid, nowTime})
            End If
        ...
        ElseIf statDataChild = statusData.Edit Then
            ...
            value.AddRange(New Object() {idDataChild, idData, "UNIT", tePhone.Text, teExtension.Text, luePhoneType.EditValue, userid, nowTime})
    ...
    Private Sub xtcProfile_SelectedPageChanged(sender As Object, e As TabPageChangedEventArgs) Handles xtcProfile.SelectedPageChanged
