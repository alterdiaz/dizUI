...
Public Class frmAbsensiKaryawan
    ...
    Private Sub deTanggal_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles lueUnit.EditValueChanged, lueDepartment.EditValueChanged, deTanggal.EditValueChanged
        ...
        Try
            If lueDepartment.EditValue Is Nothing Then
                ...
                sqls.CallSP("spGetAbsensiKaryawanUnit", "data", field, value)
                ...
                gvData.ViewCaption = "Absensi Karyawan - " & NamaBulan(CDate(deTanggal.EditValue).Month) & " " & CDate(deTanggal.EditValue).Year & " " & lueUnit.Text
            Else
                Dim sqls As New SQLs(dbstring)
                Dim field As New List(Of String)
                Dim value As New List(Of Object)
                ...
                sqls.CallSP("spGetAbsensiKaryawanUnitDept", "data", field, value)
                Threading.Thread.Sleep(200)
    ...
    Private Sub btnExportFormat_Click(sender As Object, e As EventArgs) Handles btnExportFormat.Click
        ...
        Try
            If lueDepartment.EditValue Is Nothing Then
                ...
                field.AddRange(New String() {"@periode", "@idunit"})
                value.AddRange(New Object() {Format(CDate(deTanggal.EditValue), "MMyyyy"), lueUnit.EditValue})
                sqls.CallSP("spGetAbsensiKaryawanUnitExp", "data", field, value)
                Threading.Thread.Sleep(200)
            ...
            Else
                ...
                field.AddRange(New String() {"@periode", "@iddept", "@idunit"})
                value.AddRange(New Object() {Format(CDate(deTanggal.EditValue), "MMyyyy"), lueDepartment.EditValue, lueUnit.EditValue})
                sqls.CallSP("spGetAbsensiKaryawanUnitDeptExp", "data", field, value)
                Threading.Thread.Sleep(200)
            ...
            For i As Integer = 0 To dt.Rows.Count - 1
                Try
                    ...
                    For a As Integer = 6 To dt.Columns.Count - 1
                        ...
                        If tmplist.Length <> 3 Then
                            tmpstr = "0"
                        End If
                        If IsNumeric(tmplist(0)) = False Then
                            tmpstr = "0"
                        Else
                            If CInt(tmplist(0)) > CInt(23) Then
                                tmpstr = "0"
                            Else
                                tmpstr = tmplist(0) & ":"
                            End If
                        End If
                        If IsNumeric(tmplist(1)) = False Then
                            tmpstr = "0"
                        Else
                            If CInt(tmplist(1)) < CInt(0) Or CInt(tmplist(1)) > CInt(59) Then
                                tmpstr = "0"
                            Else
                                tmpstr = tmplist(0) & ":" & tmplist(1)
                            End If
                        End If
                        If IsNumeric(tmplist(2)) = False Then
                            tmpstr = "0"
                        Else
                            If CInt(tmplist(2)) > CInt(59) Or CInt(tmplist(2)) < CInt(0) Then
                                tmpstr = "0"
                            Else
                                tmpstr = tmplist(0) & ":" & tmplist(1) & ":" & tmplist(2)
                            End If
                        End If
                        If tmpstr <> "0" And tmpstr <> "00:00:00" Then
                            Dim tmpdate2 As String = Format(cntday, "0#") & "-" & tmpdate & " " & tmpstr
                            Dim tmpdate3 As Date = Strdatetime2Datetime(tmpdate2)
                            ...
                            If sqls.getDataSet("getidjad") > 0 Then
                                ...
                                If sqls.getDataSet("cekreview") > 0 Then
                                    field.AddRange(New String() {"idstaffjadwal", "hadirdate", "updateddate", "updatedby"})
                                    value.AddRange(New Object() {iddata, tmpdate3, nowTime, userid})
    ...
    Private Sub btnImportPulang_Click(sender As Object, e As EventArgs) Handles btnImportPulang.Click
        If deTanggal.EditValue Is Nothing Then
            dizMsgbox("Periode belum dipilih", dizMsgboxStyle.Info, Me)
            Exit Sub
        End If
                    ...
                    For a As Integer = 5 To dt.Columns.Count - 1
                        ...
                        If tmpstr <> "0" And tmpstr <> "00:00:00" Then
                            ...
                            If sqls.getDataSet("getidjad") > 0 Then
                                ...
                                sqls.DMLQuery("select idstaffjadwal from staffjadwal where isreviewed=1 and idstaffjadwal='" & iddata & "'", "cekreview")
                                If sqls.getDataSet("cekreview") > 0 Then
                                    field.AddRange(New String() {"idstaffjadwal", "pulangdate", "updateddate", "updatedby"})
                                    value.AddRange(New Object() {iddata, tmpdate3, nowTime, userid})
                                    If dtsqls.datasetSave("staffjadwal", iddata, field, value, False) = True Then
                                        cnt += 1
                                    End If
