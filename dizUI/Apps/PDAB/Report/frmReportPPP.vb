Public Class frmReportPPP
    ...
    Private Sub deTanggal_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles deTanggal.EditValueChanged, lueBank.EditValueChanged
        If lueBank.EditValue IsNot Nothing And deTanggal.EditValue IsNot Nothing Then
            ...
            mys.DMLQuery("select isnull((select isnull(sum(isnull(dt.jumlahuang,0)),0) as totalkredit from aruspengajuan hd left join aruspengajuandt dt on hd.idaruspengajuan=dt.idaruspengajuan and dt.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID')  where hd.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and isnull(hd.iddiketahui,-1)<>-1 and hd.idbank='" & lueBank.EditValue & "' and dt.posisidk=2 and dt.isdeleted=0 and hd.isdeleted=0 and year(hd.tanggalpemohon)='" & Format(CDate(deTanggal.EditValue), "yyyy") & "' and LEFT(CONVERT(VARCHAR, hd.tanggalpemohon, 120), 10) < '" & Format(CDate(deTanggal.EditValue), "yyyy-MM-dd") & "' )+(select isnull(sum(isnull(-1 * dt.jumlahuang,0)),0) as totaldebet from aruspengajuan hd left join aruspengajuandt dt on hd.idaruspengajuan=dt.idaruspengajuan and dt.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') where hd.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and isnull(hd.iddiketahui,-1)<>'-1' and hd.idbank='" & lueBank.EditValue & "' and dt.posisidk=1 and dt.isdeleted=0 and hd.isdeleted=0 and year(hd.tanggalpemohon)='" & Format(CDate(deTanggal.EditValue), "yyyy") & "' and LEFT(CONVERT(VARCHAR, hd.tanggalpemohon, 120), 10) < '" & Format(CDate(deTanggal.EditValue), "yyyy-MM-dd") & "'),0) as saldosebelum", "saldosebelum")
            ...
            mys.DMLQuery("select isnull((select isnull(sum(isnull(dt.jumlahuang,0)),0) as totalkredit from aruspengajuan hd left join aruspengajuandt dt on hd.idaruspengajuan=dt.idaruspengajuan and dt.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') where hd.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and isnull(hd.iddiketahui,-1)<>-1 and hd.idbank='" & lueBank.EditValue & "' and dt.posisidk=2 and dt.isdeleted=0 and hd.isdeleted=0 and year(hd.tanggalpemohon)='" & Format(CDate(deTanggal.EditValue), "yyyy") & "' and LEFT(CONVERT(VARCHAR, hd.tanggalpemohon, 120), 10) <= '" & Format(CDate(deTanggal.EditValue), "yyyy-MM-dd") & "' )+(select isnull(sum(isnull(-1 * dt.jumlahuang,0)),0) as totaldebet from aruspengajuan hd left join aruspengajuandt dt on hd.idaruspengajuan=dt.idaruspengajuan and dt.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') where hd.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and isnull(hd.iddiketahui,-1)<>-1 and hd.idbank='" & lueBank.EditValue & "' and dt.posisidk=1 and dt.isdeleted=0 and hd.isdeleted=0 and year(hd.tanggalpemohon)='" & Format(CDate(deTanggal.EditValue), "yyyy") & "' and LEFT(CONVERT(VARCHAR, hd.tanggalpemohon, 120), 10) <= '" & Format(CDate(deTanggal.EditValue), "yyyy-MM-dd") & "' ),0) as saldosesudah", "saldosesudah")
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        Dim mys As New SQLs(dbstring)
        mys.DMLQuery("select * from xrppp where idbank='" & lueBank.EditValue & "' and LEFT(CONVERT(VARCHAR, tanggalpemohon, 120), 10) = '" & Format(CDate(deTanggal.EditValue), "yyyy-MM-dd") & "' order by posisidk asc,tanggalpemohon asc", "xrppp")
        ...
        rpt.DataSource = mys.dataTable("xrppp")
        rpt.DataMember = "xrppp"
