Public Class frmRenumberingDocument
    ...
    Public Sub setParameter(ByVal tblstr As String, ByVal yearstr As String, ByVal formatstr As String, ByVal fieldno As String, ByVal fieldid As String, ByVal fielddate As String, ByVal positionstring As String, ByVal lengthnumber As Integer)
        Me.tableName = tblstr
        Me.tbYear = yearstr
    ...
    Private Sub calculate()
        If Me.tbName = "" Then
            MsgBox("Table Name not set", MsgBoxStyle.Critical, "Error")
            Exit Sub
        End If
        If Me.strYear = "" Then
            MsgBox("Year not set", MsgBoxStyle.Critical, "Error")
            Exit Sub
        End If
        If Me.formatStr = "" Then
            MsgBox("Format String not set", MsgBoxStyle.Critical, "Error")
            Exit Sub
        End If
        If Me.fldno = "" Then
            MsgBox("Field No not set", MsgBoxStyle.Critical, "Error")
            Exit Sub
        End If
        If Me.fldid = "" Then
            MsgBox("Field ID not set", MsgBoxStyle.Critical, "Error")
            Exit Sub
        End If
        If Me.flddt = "" Then
            MsgBox("Field Date not set", MsgBoxStyle.Critical, "Error")
            Exit Sub
        End If
        If Me.positionStr = "" Then
            MsgBox("Position not set", MsgBoxStyle.Critical, "Error")
            Exit Sub
        End If
        If Me.lenNumber = 0 Then
            MsgBox("Length Number not set", MsgBoxStyle.Critical, "Error")
            Exit Sub
        End If
        ...
        lblNumber.Text = "Calculating Documents for : " & vbCrLf & tbName & strYear & " has " & counter & " rows"
        mys.DMLQuery("DELETE FROM sys_generatenumber where tablename='" & tbName & strYear & "'", False)
        ...
        For i As Integer = 0 To counter - 1
            lastNo = generateNumber(Me.tbName & Me.strYear, Me.formatStr, Me.lenNumber, Me.positionStr)

            mydt = New dtsetSQLS(dbstring)
            lblNumber.Text = "Calculating Documents for : " & vbCrLf & _
            tbName & strYear & " has " & counter & " rows" & vbCrLf & _
            "Current index on " & i
    ...
    Private Sub frmRenumberingDocument_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        calculate()
    End Sub
