Public Class frmReviewKabidKeuPUMPB
    Const HTCAPTION = &H2
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        If idselect <> "" Then
            ...
            If bdiketahui = False Then
                field.AddRange(New String() {"idpengajuan", "idkabid", "namakabid", "jabatankabid", "deptkabid", "tanggalkabid", "isdeleted", "rejectreason"})
            Else
                field.AddRange(New String() {"idpengajuan", "idkabid", "namakabid", "jabatankabid", "deptkabid", "tanggalkabid", "namadirektur", "jabatandirektur", "deptdirektur", "isdeleted", "rejectreason"})
            End If
            If ceReject.EditValue = False Then
                If bdiketahui = False Then
                    value.AddRange(New Object() {idselect, staffid, staffname, levelname, deptname, nowTime, 0, "-"})
                Else
                    value.AddRange(New Object() {idselect, staffid, staffname, levelname, deptname, nowTime, namadiketahui, jabatandiketahui, deptdiketahui, 0, "-"})
                End If
            Else
                If bdiketahui = False Then
                    value.AddRange(New Object() {idselect, staffid, staffname, levelname, deptname, nowTime, 1, teAlasan.Text})
                Else
                    value.AddRange(New Object() {idselect, staffid, staffname, levelname, deptname, nowTime, namadiketahui, jabatandiketahui, deptdiketahui, 1, teAlasan.Text})
                End If
            End If
