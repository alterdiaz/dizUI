Public Class frmMonUserPPP
    ...
    Private Sub frmMonPPP_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        sqls.DMLQuery("select b.idbank,b.bank,b.norekening,b.remarks from bank b where isnull(b.isdeleted,0)=0 order by b.bank asc,b.norekening asc", "bank")
    ...
    Private Sub deTanggal_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles deTanggal.EditValueChanged, lueBank.EditValueChanged
        ...
        If lueBank.EditValue IsNot Nothing And deTanggal.EditValue IsNot Nothing Then
            ...
            mys.DMLQuery("select hd.noaruspengajuan as 'No Pengajuan',gchd.generalcode as 'Status Pengajuan',dt.nomorurut as 'Nomor Urut',dt.keperluan as Keperluan,dt.nodokumen as 'No Dokumen',dt.jumlahuang as 'Jumlah Uang',pdk.generalcode as 'Debet/Kredit',gcdt.generalcode as 'Status Detail' from aruspengajuan hd left join bank b on hd.idbank=b.idbank left join aruspengajuandt dt on hd.idaruspengajuan=dt.idaruspengajuan left join sys_generalcode pdk on dt.posisidk=pdk.idgeneral and pdk.gctype='POSISIDK' left join sys_generalcode gchd on hd.isdeleted=gchd.idgeneral and gchd.gctype='DELETE' left join sys_generalcode gcdt on dt.isdeleted=gcdt.idgeneral and gcdt.gctype='DELETE' where hd.idpemohon='" & userid & "' and isnull(hd.iddiketahui,'-1')<>'-1' and hd.idbank='" & lueBank.EditValue & "' and LEFT(CONVERT(VARCHAR, hd.tanggalpemohon, 120), 10) = '" & Format(CDate(deTanggal.EditValue), "yyyy-MM-dd") & "' order by dt.posisidk asc,hd.tanggalpemohon asc", "data")
