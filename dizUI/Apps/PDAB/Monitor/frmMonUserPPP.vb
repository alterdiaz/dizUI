Public Class frmMonUserPPP
    ...
    Private Sub frmMonPPP_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Activated
        gvData.Focus()
    End Sub

    Private Sub frmMonPPP_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If formTitle <> "" Then
            Me.Text = formTitle
        ...
        sqls.DMLQuery("select b.idbank,b.bank,b.norekening,b.remarks from bank b where b.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and isnull(b.isdeleted,0)=0 order by b.bank asc,b.norekening asc", "bank")
    ...
    Private Sub deTanggal_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles deTanggal.EditValueChanged, lueBank.EditValueChanged
        ...
        If lueBank.EditValue IsNot Nothing And deTanggal.EditValue IsNot Nothing Then
            ...
            mys.DMLQuery("select isnull((select isnull(sum(isnull(-1 * dt.jumlahuang,0)),0) as totaldebet from aruspengajuan hd left join aruspengajuandt dt on hd.idaruspengajuan=dt.idaruspengajuan and dt.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') where hd.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and isnull(hd.iddiketahui,-1)<>-1 and hd.idbank='" & lueBank.EditValue & "' and dt.posisidk=1 and isnull(dt.isdeleted,0)=0 and isnull(hd.isdeleted,0)=0 and year(hd.tanggalpemohon)='" & Format(CDate(deTanggal.EditValue), "yyyy") & "' and LEFT(CONVERT(VARCHAR, hd.tanggalpemohon, 120), 10) < '" & Format(CDate(deTanggal.EditValue), "yyyy-MM-dd") & "')+(select isnull(sum(isnull(dt.jumlahuang,0)),0) as totalkredit from aruspengajuan hd left join aruspengajuandt dt on hd.idaruspengajuan=dt.idaruspengajuan  and dt.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') where hd.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and isnull(hd.iddiketahui,-1)<>-1 and hd.idbank='" & lueBank.EditValue & "' and dt.posisidk=2 and dt.isdeleted=0 and hd.isdeleted=0 and year(hd.tanggalpemohon)='" & Format(CDate(deTanggal.EditValue), "yyyy") & "' and LEFT(CONVERT(VARCHAR, hd.tanggalpemohon, 120), 10) < '" & Format(CDate(deTanggal.EditValue), "yyyy-MM-dd") & "'),0) as saldosebelum", "saldosebelum")
            ...
            mys.DMLQuery("select isnull((select isnull(sum(isnull(-1 * dt.jumlahuang,0)),0) as totaldebet from aruspengajuan hd left join aruspengajuandt dt on hd.idaruspengajuan=dt.idaruspengajuan and dt.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') where hd.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and isnull(hd.iddiketahui,-1)<>-1 and hd.idbank='" & lueBank.EditValue & "' and dt.posisidk=1 and isnull(dt.isdeleted,0)=0 and hd.isdeleted=0 and year(hd.tanggalpemohon)='" & Format(CDate(deTanggal.EditValue), "yyyy") & "' and LEFT(CONVERT(VARCHAR, hd.tanggalpemohon, 120), 10) <= '" & Format(CDate(deTanggal.EditValue), "yyyy-MM-dd") & "')+(select isnull(sum(isnull(dt.jumlahuang,0)),0) as totalkredit from aruspengajuan hd left join aruspengajuandt dt on hd.idaruspengajuan=dt.idaruspengajuan and dt.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') where hd.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and isnull(hd.iddiketahui,-1)<>-1 and hd.idbank='" & lueBank.EditValue & "' and dt.posisidk=2 and dt.isdeleted=0 and hd.isdeleted=0 and year(hd.tanggalpemohon)='" & Format(CDate(deTanggal.EditValue), "yyyy") & "' and LEFT(CONVERT(VARCHAR, hd.tanggalpemohon, 120), 10) <= '" & Format(CDate(deTanggal.EditValue), "yyyy-MM-dd") & "'),0) as saldosesudah", "saldosesudah")
            ...
            mys.DMLQuery("select hd.noaruspengajuan as 'No Pengajuan',gchd.generalcode as 'Status Pengajuan',dt.nomorurut as 'Nomor Urut',dt.keperluan as Keperluan,dt.nodokumen as 'No Dokumen',dt.jumlahuang as 'Jumlah Uang',pdk.generalcode as 'Debet/Kredit',gcdt.generalcode as 'Status Detail' from aruspengajuan hd left join bank b on hd.idbank=b.idbank and b.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') left join aruspengajuandt dt on hd.idaruspengajuan=dt.idaruspengajuan and dt.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') left join sys_generalcode pdk on dt.posisidk=pdk.idgeneral and pdk.gctype='POSISIDK' left join sys_generalcode gchd on hd.isdeleted=gchd.idgeneral and gchd.gctype='DELETE' left join sys_generalcode gcdt on dt.isdeleted=gcdt.idgeneral and gcdt.gctype='DELETE' where hd.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and hd.idpemohon='" & userid & "' and isnull(hd.iddiketahui,-1)<>-1 and hd.idbank='" & lueBank.EditValue & "' and LEFT(CONVERT(VARCHAR, hd.tanggalpemohon, 120), 10) = '" & Format(CDate(deTanggal.EditValue), "yyyy-MM-dd") & "' order by dt.posisidk asc,hd.tanggalpemohon asc", "data")
            ...
            For Each gridcol As DevExpress.XtraGrid.Columns.GridColumn In gvData.Columns
                ...
                End If
                If gridcol.ColumnType.FullName = "System.Decimal" Then
