Public Class frmMonPPP
    ...
    Private Sub frmMonPPP_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        sqls.DMLQuery("select b.idbank,b.bank,b.norekening,b.remarks from bank b where b.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and b.isdeleted=0", "bank")
    ...
    Private Sub deTanggal_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles deTanggal.EditValueChanged, lueBank.EditValueChanged
        If lueBank.EditValue IsNot Nothing And deTanggal.EditValue IsNot Nothing Then
            ...
            mys.DMLQuery("select isnull((select isnull(sum(isnull(-1 * dt.jumlahuang,0)),0) as totaldebet from aruspengajuan hd left join aruspengajuandt dt on hd.idaruspengajuan=dt.idaruspengajuan where hd.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and isnull(hd.iddiketahui,-1)<>-1 and hd.idbank='" & lueBank.EditValue & "' and dt.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and dt.posisidk=1 and dt.isdeleted=0 and hd.isdeleted=0 and year(hd.tanggalpemohon)='" & Format(CDate(deTanggal.EditValue), "yyyy") & "' and LEFT(CONVERT(VARCHAR, hd.tanggalpemohon, 120), 10) < '" & Format(CDate(deTanggal.EditValue), "yyyy-MM-dd") & "')+(select isnull(sum(isnull(dt.jumlahuang,0)),0) as totalkredit from aruspengajuan hd left join aruspengajuandt dt on hd.idaruspengajuan=dt.idaruspengajuan where hd.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and isnull(hd.iddiketahui,-1)<>-1 and hd.idbank='" & lueBank.EditValue & "' and dt.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and dt.posisidk=2 and dt.isdeleted=0 and hd.isdeleted=0 and year(hd.tanggalpemohon)='" & Format(CDate(deTanggal.EditValue), "yyyy") & "' and LEFT(CONVERT(VARCHAR, hd.tanggalpemohon, 120), 10) < '" & Format(CDate(deTanggal.EditValue), "yyyy-MM-dd") & "'),0) as saldosebelum", "saldosebelum")
            ...
            mys.DMLQuery("select isnull((select isnull(sum(isnull(-1 * dt.jumlahuang,0)),0) as totaldebet from aruspengajuan hd left join aruspengajuandt dt on hd.idaruspengajuan=dt.idaruspengajuan where hd.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and isnull(hd.iddiketahui,-1)<>-1 and hd.idbank='" & lueBank.EditValue & "' and dt.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and dt.posisidk=1 and dt.isdeleted=0 and hd.isdeleted=0 and year(hd.tanggalpemohon)='" & Format(CDate(deTanggal.EditValue), "yyyy") & "' and LEFT(CONVERT(VARCHAR, hd.tanggalpemohon, 120), 10) <= '" & Format(CDate(deTanggal.EditValue), "yyyy-MM-dd") & "')+(select isnull(sum(isnull(dt.jumlahuang,0)),0) as totalkredit from aruspengajuan hd left join aruspengajuandt dt on hd.idaruspengajuan=dt.idaruspengajuan where hd.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and isnull(hd.iddiketahui,-1)<>-1 and hd.idbank='" & lueBank.EditValue & "' and dt.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and dt.posisidk=2 and dt.isdeleted=0 and hd.isdeleted=0 and year(hd.tanggalpemohon)='" & Format(CDate(deTanggal.EditValue), "yyyy") & "' and LEFT(CONVERT(VARCHAR, hd.tanggalpemohon, 120), 10) <= '" & Format(CDate(deTanggal.EditValue), "yyyy-MM-dd") & "'),0) as saldosesudah", "saldosesudah")
