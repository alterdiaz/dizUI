...
Public Class frmAppointment
    ...
    Dim lenParamedis As Integer = 400
    ...
    Private Sub lueParamedis_EditValueChanged(sender As Object, e As EventArgs) Handles lueParamedis.EditValueChanged
        ...
        sqls.DMLQuery("select s.idspesialis as id,s.nama as content from spesialis s left join paramedisspesialis ps on s.idspesialis=ps.idspesialis and ps.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') where s.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and ps.idparamedis='" & lueParamedis.EditValue & "' order by s.nama asc", "paramedisspesialis2")
        ...
        sqls.DMLQuery("select top 2 case when phonetype=1 then isnull(phone + ' (' + extension +')','-') else isnull(phone,'-') end as phone from phone where idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and tablereff='PARAMEDIS' and isdeleted=0 and isprimary=1 and idreff='" & lueParamedis.EditValue & "' order by idphone asc", "phone")
    ...
    Private Sub lueSpesialis_EditValueChanged(sender As Object, e As EventArgs) Handles lueSpesialis.EditValueChanged
        ...
        Dim mysqls As New SQLs(dbstring)
        mysqls.DMLQuery("select pj.tanggalmulai,pj.tanggalselesai,replace(pj.hari,' ','') hari from paramedisjadwal pj left join paramedisspesialis ps on pj.idparamedisspesialis=ps.idparamedisspesialis and ps.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID')  where pj.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and (getdate() >= pj.tanggalmulai and getdate() <= pj.tanggalselesai) and ps.idparamedis='" & lueParamedis.EditValue & "' and ps.idspesialis='" & lueSpesialis.EditValue & "'", "hari")
        ...
        mysqls.DMLQuery("select top 1 convert(varchar,pj.tanggalselesai,105) as tanggalselesai from paramedisjadwal pj left join paramedisspesialis ps on pj.idparamedisspesialis=ps.idparamedisspesialis and ps.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') where pj.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and (getdate() >= pj.tanggalmulai and getdate() <= pj.tanggalselesai) and ps.idparamedis='" & lueParamedis.EditValue & "' and ps.idspesialis='" & lueSpesialis.EditValue & "' order by pj.tanggalselesai desc", "tglmax")
    ...
    Private Sub buttonClick(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim mys As New SQLs(dbstring)
        mys.DMLQuery("select top 1 idvacation,kehadiran,remarks from vacation where idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and convert(varchar,vacationdate,105)='" & strDate & "' and isdeleted=0 and kehadiran<>0 and idreff='" & lueParamedis.EditValue & "' and tablereff='PARAMEDIS' order by idvacation desc", "cekijin")
        ...
        mys.DMLQuery("select pj.idjadwalsesi as id,js.nama +' ('+ substring(convert(varchar,ISNULL(jp.jammulaipengganti,pj.jammulai),108),0,6) + ' - ' +substring(convert(varchar,ISNULL(jp.jamselesaipengganti,pj.jamselesai),108),0,6)+')' as content from paramedisjadwal pj left join paramedisspesialis ps on pj.idparamedisspesialis=ps.idparamedisspesialis and ps.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') left join jadwalsesi js on pj.idjadwalsesi=js.idjadwalsesi and js.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') left join jadwalparamedis jp on ps.idparamedis=jp.idparamedis and convert(varchar,jp.jadwaltanggal,105)='" & Format(dateDate, "dd-MM-yyyy") & "' and jp.idjadwalsesi=pj.idjadwalsesi and jp.isdeleted=0 where pj.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and (convert(date,'" & Format(dateDate, "yyyy-MM-dd") & "') >= pj.tanggalmulai and convert(date,'" & Format(dateDate, "yyyy-MM-dd") & "') <= pj.tanggalselesai) and pj.hari like '%" & dateDate.DayOfWeek & "%' and ps.idparamedis='" & lueParamedis.EditValue & "' and ps.idspesialis='" & lueSpesialis.EditValue & "'", "jadwalsesi")
    ...
    Private Sub slueLamaNama_EditValueChanged(sender As Object, e As EventArgs) Handles slueLamaNama.EditValueChanged
        ...
        Dim sqls As New SQLs(dbstring)
        sqls.DMLQuery("select r.idrekammedis,r.jeniskelamin as idjeniskelamin,jk.generalcode as jeniskelamin,convert(varchar,r.tanggallahir,105) as tanggallahir,isnull(a.alamat,'-') as alamat,isnull(t.phone,0) as phone from rekammedis r left join sys_generalcode jk on r.jeniskelamin=jk.idgeneral and jk.gctype='SEXTYPE' left join (select idreff,alamat from alamat where tablereff='PASIEN' and isdeleted=0 and isprimary=1 and idcompany=(select top 1 value from sys_appsetting where variable='CompanyID')) a on r.idrekammedis=a.idreff left join (select idreff,phone from phone where tablereff='PASIEN' and isdeleted=0 and isprimary=1 and idcompany=(select top 1 value from sys_appsetting where variable='CompanyID')) t on r.idrekammedis=t.idreff where r.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and r.idrekammedis='" & slueLamaNama.EditValue & "'", "data")
        ...
        If sqls.getDataSet("data") > 0 Then
            teLamaTelepon1.Text = sqls.getDataSet("data", 0, "phone")
        Else
            teLamaTelepon1.Text = "0"
        End If
        If sqls.getDataSet("data") > 1 Then
            teLamaTelepon2.Text = sqls.getDataSet("data", 1, "phone")
        Else
            teLamaTelepon2.Text = "0"
        End If
        teLamaRemarks.Text = "-"
    ...
    Private Sub dnKalender_EditValueChanged(sender As Object, e As EventArgs) Handles dnKalender.EditValueChanged
        ...
        If isButton = False Then
            ...
            Dim mys As New SQLs(dbstring)
            mys.DMLQuery("select top 1 idvacation,kehadiran,remarks from vacation where idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and convert(varchar,vacationdate,105)='" & strDate & "' and isdeleted=0 and kehadiran<>0 and idreff='" & lueParamedis.EditValue & "' and tablereff='PARAMEDIS' order by createddate desc", "cekijin")
            If mys.getDataSet("cekijin") > 0 Then
                If mys.getDataSet("cekijin", 0, "kehadiran") = 1 Then
                    showPanelInfo(InfoType.info, "Tidak Hadir - Sakit", mys.getDataSet("cekijin", 0, "remarks"))
                ElseIf mys.getDataSet("cekijin", 0, "kehadiran") = 2 Then
    ...
    Private Sub lueJadwalSesi_EditValueChanged(sender As Object, e As EventArgs) Handles lueJadwalSesi.EditValueChanged
        ...
        Dim sqls As New SQLs(dbstring)
        sqls.DMLQuery("select top 1 idvacation,kehadiran,remarks from vacation where idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and convert(varchar,vacationdate,105)='" & strDate & "' and isdeleted=0 and kehadiran<>0 and idjadwalsesi='" & lueJadwalSesi.EditValue & "' and idreff='" & lueParamedis.EditValue & "' and tablereff='PARAMEDIS' order by idvacation desc", "cekijin")
        ...
        sqls.DMLQuery("select substring(convert(varchar,ISNULL(jp.jammulaipengganti,pj.jammulai),108),0,6) as jammulai,substring(convert(varchar,ISNULL(jp.jamselesaipengganti,pj.jamselesai),108),0,6) as jamselesai,pj.maxapp,pj.durasi from paramedisjadwal pj left join paramedisspesialis ps on pj.idparamedisspesialis=ps.idparamedisspesialis and ps.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') left join jadwalparamedis jp on ps.idparamedis=jp.idparamedis and convert(varchar,jp.jadwaltanggal,105)='" & strDate & "' and jp.idjadwalsesi='" & lueJadwalSesi.EditValue & "' and jp.isdeleted=0  where pj.hari like '%" & thisDate.DayOfWeek & "%' and pj.idparamedisspesialis=(select idparamedisspesialis from paramedisspesialis where idparamedis='" & lueParamedis.EditValue & "' and idspesialis='" & lueSpesialis.EditValue & "')", "cektgl")
