Public Class frmParamedis
    ...
    Private Sub loadLOV()
        ...
        mysqls.DMLQuery("select idjadwalsesi as id,nama + ' (' + substring(convert(varchar,jammulai,108),0,6)+' - ' + substring(convert(varchar,jamselesai,108),0,6)+')' as content from jadwalsesi where isnull(isdeleted,0)=0 order by priority asc", "sesi")
        ...
        mysqls.DMLQuery("select idlokasi as id,nama as content from lokasi where iddepartment in (select [value] from sys_appsetting where variable='IDIRJDept') order by nama asc", "jadwallokasi")
        ...
        mysqls.DMLQuery("select ps.idparamedisspesialis as id,s.nama as content,s.remarks as remarks from paramedisspesialis ps left join spesialis s on ps.idspesialis=s.idspesialis where 1=0 order by s.remarks asc", "mparamspesialis")
        ...
        mysqls.DMLQuery("select idspesialis as id, remarks as content,nama as remarks from spesialis where isnull(isdeleted,0)=0 order by remarks asc", "mspesialis")
        ...
        mysqls.DMLQuery("select u.iduser,u.username,l.userlevel from sys_user u left join sys_userlevel l on u.iduserlevel=l.iduserlevel where u.username<>'superadmin'", "user")
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If statData = statusData.Baru Then
            ...
            sqls.DMLQuery("select nama from paramedis where replace(nama,' ','')='" & teParamedis.Text.Replace(" ", "") & "'", "cek")
            ...
            Else
                ...
                Exit Sub
            End If
            sqls.DMLQuery("select nama from paramedis where (iduser<>0 and iduser='" & lueUsername.EditValue & "')", "cekuser")
        ...
        ElseIf statData = statusData.Edit Then
            Dim sqls As New SQLs(dbstring)
            sqls.DMLQuery("select nama from paramedis where replace(nama,' ','')='" & teParamedis.Text.Replace(" ", "") & "' and idparamedis<>'" & idData & "'", "cek")
            If sqls.getDataSet("cek") > 0 Then
                dizMsgbox("Data tersebut sudah ada", dizMsgboxStyle.Info, Me)
                teParamedis.Focus()
                Exit Sub
            End If
            sqls.DMLQuery("select nama from paramedis where (iduser<>0 and iduser='" & lueUsername.EditValue & "') and idparamedis<>'" & idData & "'", "cekuser")
            If sqls.getDataSet("cekuser") > 0 Then
                dizMsgbox("Username tersebut sudah dipakai", dizMsgboxStyle.Info, Me)
                teParamedis.Focus()
                Exit Sub
    ...
    Private Sub loadSpesialis(idreff As String)
        ...
        fromGridChild = False
        Dim sqls As New SQLs(dbstring)
        sqls.DMLQuery("select ps.idparamedisspesialis,ps.idspesialis,s.nama as spesialis,ps.remarks,ps.isdeleted,del.generalcode as statdata from paramedisspesialis ps left join spesialis s on ps.idspesialis=s.idspesialis left join sys_generalcode del on ps.isdeleted=del.idgeneral and del.gctype='DELETE' where ps.idparamedis='" & idreff & "' order by s.nama asc,ps.isdeleted asc", "paramspesialis")
    ...
    Private Sub loadParamSpesialis(idreff As String)
        ...
        sqls.DMLQuery("select ps.idparamedisspesialis as id,s.nama as content,s.remarks as remarks from paramedisspesialis ps left join spesialis s on ps.idspesialis=s.idspesialis where isnull(ps.isdeleted,0)=0 and ps.idparamedis='" & idreff & "' order by s.remarks asc", "mparamspesialis")
    ...
    Private Sub btnSaveEmail_Click(sender As Object, e As EventArgs) Handles btnSaveEmail.Click
        ...
        If statDataChild = statusData.Baru Then
            Dim sqls As New SQLs(dbstring)
            sqls.DMLQuery("select email from email where tablereff='PARAMEDIS' and idreff='" & idData & "' and email='" & teEmail.Text & "'", "cek")
            If sqls.getDataSet("cek") = 0 Then
                idDatachild = "-1"
            Else
    ...
    Private Sub btnSaveJadwal_Click(sender As Object, e As EventArgs) Handles btnSaveJadwal.Click
        ...
        If statDataChild = statusData.Baru Then
            ...
            Next
            Dim sqls As New SQLs(dbstring)
            sqls.DMLQuery("select pj.idparamedisjadwal from paramedisjadwal pj left join paramedisspesialis ps on pj.idparamedisspesialis=ps.idparamedisspesialis where isnull(pj.isdeleted,0)=0 and ps.idparamedis='" & idData & "' and ('" & strjam & ":" & strmenit & ":00' between pj.jammulai and pj.jamselesai)" & wherehari, "cek")
            If sqls.getDataSet("cek") = 0 Then
    ...
    Private Sub btnSaveRegister_Click(sender As Object, e As EventArgs) Handles btnSaveRegister.Click
        ...
        If statDataChild = statusData.Baru Then
            Dim sqls As New SQLs(dbstring)
            sqls.DMLQuery("select nomor from pelatihan where tablereff='PARAMEDIS' and idreff='" & idData & "'", "cek")
            If sqls.getDataSet("cek") = 0 Then
                idDataChild = GenerateGUID() '-1
            Else
    ...
    Private Sub lueParamedisSpesialis_EditValueChanged(sender As Object, e As EventArgs) Handles lueParamedisSpesialis.EditValueChanged
        ...
        Dim sqls As New SQLs(dbstring)
        Dim idsp As String = ""
        sqls.DMLQuery("select idspesialis from paramedisspesialis where idparamedisspesialis='" & lueParamedisSpesialis.EditValue & "'", "ids")
        idsp = sqls.getDataSet("ids", 0, "idspesialis")
        sqls.DMLQuery("select idlokasi as id,nama as content from lokasi where nama = (select case when variable='IDSPPK' then 'Laboratorium' when variable='IDSPRad' then 'Radiologi' end as 'department' from sys_appsetting where variable in ('IDSPPK','IDSPRad') and [value]='" & idsp & "')", "jadwallokasi")
        ...
        If sqls.getDataSet("jadwallokasi") = 0 Then
            ...
            sqls.DMLQuery("select idlokasi as id,nama as content from lokasi where iddepartment in (select [value] from sys_appsetting where variable='IDIRJDept') order by nama asc", "jadwallokasi2")
