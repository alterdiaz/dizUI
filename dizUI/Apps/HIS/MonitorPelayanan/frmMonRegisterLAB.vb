Public Class frmMonRegisterLAB
    Const HTCAPTION = &H2
    ...
    Private Sub deTanggal_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles deTanggal.EditValueChanged
        ...
        sqls.CallSP("spMonRegisterLAB", "monreglab", field, value)
        dttbl = New DataTable
        dttbl = sqls.dataTable("monreglab")

        'For i As Integer = 0 To gvData.RowCount - 1
        '    gvData.SetRowCellValue(i, GridColumn1, i + 1)
        'Next

        sqls.DMLQuery("select distinct im.itemmonitor from itemmonitor im where im.iddepartment in (select value from sys_appsetting where variable='IDLABDept')", "getim")
    ...
    Private Sub Form_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Dim sqls As New SQLs(dbstring)
        sqls.DMLQuery("select distinct im.itemmonitor from itemmonitor im where im.iddepartment in (select value from sys_appsetting where variable='IDLABDept') order by itemmonitor asc", "getim")
    ...
    Private Sub btnItemMonitor_Click(sender As Object, e As EventArgs) Handles btnItemMonitor.Click
        Dim sqls As New SQLs(dbstring)
        sqls.DMLQuery("select value from sys_appsetting where variable='IDLABDept'", "getdept")
