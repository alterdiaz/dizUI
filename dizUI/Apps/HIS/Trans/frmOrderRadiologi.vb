Public Class frmOrderRadiologi
    Const HTCAPTION = &H2
    ...
    Private Sub loadItem()
        ...
        value.AddRange(New Object() {idunit, "7,12"})
    ...
    Private Sub frmOrderRadiologi_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If _idreginduk <> "" Then
            ...
            Else
                ...
                Else
                    dizMsgbox("Dokter Baca Radiologi tidak ditemukan", dizMsgboxStyle.Info, Me)
                    Me.Dispose()
                End If
                ...
                Else
                    dizMsgbox("Dokter Baca Radiologi tidak ada yang praktek hari ini", dizMsgboxStyle.Info, Me)
                    Me.Dispose()
                End If

                sqls.DMLQuery("select idrekammedis,payertype,asalpx,asalpx2,idpayer,idasuransi,ishamil,iddokterruangan,idspesialisruangan,idkelas from registrasi where idregistrasi='" & _idreginduk & "'", "getidrm")
                ...
                iddokterpengirim = sqls.getDataSet("getidrm", 0, "iddokterruangan")
                idspesialispengirim = sqls.getDataSet("getidrm", 0, "idspesialisruangan")
                ...
                Else
                    dizMsgbox("Ruang Radiologi tidak ditemukan", dizMsgboxStyle.Info, Me)
                    Me.Dispose()
                End If
                ...
                sqls.DMLQuery("select count(idregistrasi) as count from registrasi where isdeleted=0 and iddokterruangan='" & idparamedis & "' and idrekammedis='" & idrm & "'", "cntdokter")
                cntdokter = sqls.getDataSet("cntdokter", 0, "count")

                Dim dsqls As New dtsetSQLS(dbstring)
                Dim field As New List(Of String)
                Dim value As New List(Of Object)
                field.AddRange(New String() {"idregistrasi", "idregistrasiparent", "transactiontype", "idjadwalsesi", "idcompany", "iddepartment", "idunit", "transactiontype", "idrekammedis", "idlokasi", "idkelas", "iddokterruangan", "idspesialisruangan", "registrasino", "kunjunganke", "kunjungandokterke", "kunjungandepartmentke", "asalpx", "asalpx2", "asalrujukan", "iddokterrujukan", "idspesialisrujukan", "jeniskunjungan", "registrasistatus", "remarks", "payertype", "idpayer", "idasuransi", "ishamil", "isdeleted", "deletereason", "createdby", "isoneday"})
                value.AddRange(New Object() {idReg, _idreginduk, idRegType, idjadwalsesi, idcomp, iddept, idunit, idRegType, idrm, idlokasi, idkelas, idparamedis, idspesialis, teNoRegistrasi.Text, cnt + 1, cntdokter + 1, cntdept + 1, 0, 0, 0, iddokterpengirim, idspesialispengirim, CLng(2), 0, teRemarks.Text, payertype, idpayer, idasuransi, ishamil, 0, "-", userid, 0})
                If dsqls.datasetSave("registrasi", idReg, field, value, False) = True Then
                    dizMsgbox("Registrasi Radiologi telah dibuat", dizMsgboxStyle.Info, Me)
                End If
            End If
            tmrCek.Start()
        End If
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        value.AddRange(New Object() {idTrans, teNoTrans.Text, idTransType, idReg, idRegType, 1, "Unit", _idunitpengirim, _iddeptpengirim, "Unit", lueUnit.EditValue, iddept, idcomp, userid, nowTime, ipadd, hostname, idkelas, teRemarks.Text})
        If dsqls.datasetSave("transaksi", idTrans, field, value, False) = True Then
            ...
            Next
            If cekval = True Then
                dizMsgbox("Order Radiologi telah tersimpan", dizMsgboxStyle.Info, Me)
            End If
        End If
    End Sub
