Public Class frmParamedisJadwal
    ...
    Private Sub loadLOV()
        ...
        mysqls.DMLQuery("select s.idspesialis as id, s.nama as content,s.remarks as remarks from paramedisspesialis ps left join spesialis s on ps.idspesialis=s.idspesialis where 1=0 order by s.remarks asc", "mparamspesialis")
    ...
    Private Sub loadParamSpesialis(idreff As String)
        ...
        sqls.DMLQuery("select s.idspesialis as id, s.nama as content,s.remarks as remarks from paramedisspesialis ps left join spesialis s on ps.idspesialis=s.idspesialis where ps.idparamedis='" & idreff & "' order by s.remarks asc", "mparamspesialis")
    ...
    Private Sub btnDelete_Click(sender As Object, e As EventArgs) Handles btnDelete.Click
        If dizMsgbox("Jadwal tersebut sudah tidak berlaku?", dizMsgboxStyle.Konfirmasi, Me) = dizMsgboxValue.Batal Then
            Exit Sub
        End If
        ...
        ElseIf btnDelete.Text = "AKTIF" Then
            ...
            Next
            Dim sqls As New SQLs(dbstring)
            sqls.DMLQuery("select pj.idparamedisjadwal from paramedisjadwal pj left join paramedisspesialis ps on pj.idparamedisspesialis=ps.idparamedisspesialis where (isnull(pj.isdeleted,0)<>2 or isnull(pj.isdeleted,0)=1) and ps.idparamedis='" & idData & "' and ('" & strjam & ":" & strmenit & ":00' between pj.jammulai and pj.jamselesai)" & wherehari, "cek")
    ...
    Private Sub btnDeletePermanen_Click(sender As Object, e As EventArgs) Handles btnDeletePermanen.Click
        If dizMsgbox("Hapus Jadwal secara permanen?", dizMsgboxStyle.Konfirmasi, Me) = dizMsgboxValue.Batal Then
            Exit Sub
        End If
        ...
        ElseIf btnDelete.Text = "AKTIF" Then
            ...
            Next
            Dim sqls As New SQLs(dbstring)
            sqls.DMLQuery("select pj.idparamedisjadwal from paramedisjadwal pj left join paramedisspesialis ps on pj.idparamedisspesialis=ps.idparamedisspesialis where isnull(pj.isdeleted,0)<>2 and ps.idparamedis='" & idData & "' and ('" & strjam & ":" & strmenit & ":00' between pj.jammulai and pj.jamselesai)" & wherehari, "cek")
            If sqls.getDataSet("cek") > 0 Then
                dizMsgbox("Jadwal Paramedis tersebut sudah ada", dizMsgboxStyle.Info, Me)
                lueParamedis.Focus()
                Exit Sub
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        If statData = statusData.Baru Then
            ...
            Next
            Dim sqls As New SQLs(dbstring)
            sqls.DMLQuery("select pj.idparamedisjadwal from paramedisjadwal pj left join paramedisspesialis ps on pj.idparamedisspesialis=ps.idparamedisspesialis where pj.isdeleted<>2 and ps.idparamedis='" & lueParamedis.EditValue & "' and idjadwalsesi='" & lueJadwalSesi.EditValue & "' " & wherehari, "cek")
        ...
        ElseIf statData = statusData.Edit Then
            ...
            For i As Integer = 0 To hari.Length - 1
                wherehari &= " and (case when pj.hari like '%" & hari(i) & "%' then 'Ya' else 'Tidak' end) = 'Ya' "
            Next
            sqls.DMLQuery("select pj.idparamedisjadwal from paramedisjadwal pj left join paramedisspesialis ps on pj.idparamedisspesialis=ps.idparamedisspesialis where pj.idparamedisjadwal<>'" & idData & "' and pj.isdeleted<>2 and ps.idparamedis='" & lueParamedis.EditValue & "' and idjadwalsesi='" & lueJadwalSesi.EditValue & "' and " & wherehari, "cek")
    ...
    Private Sub lueSpesialis_EditValueChanged(sender As Object, e As EventArgs) Handles lueSpesialis.EditValueChanged
        ...
        If sqls.getDataSet("jadwallokasi") = 0 Then
            ...
            sqls.DMLQuery("select idlokasi as id,nama as content from lokasi where lokasitype=3 and iddepartment in (select [value] from sys_appsetting where variable='IDIRJDept') order by nama asc", "jadwallokasi2")
