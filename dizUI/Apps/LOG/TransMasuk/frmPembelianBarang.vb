...
Public Class frmPembelianBarang
    Const HTCAPTION = &H2
    ...
    Private Sub updateSPP()
        ...
        Dim ipadd As String = getIPAddress(ipaddparam.IP)
        Dim hostnm As String = getIPAddress(ipaddparam.Host)

        For i As Integer = 0 To sqls.getDataSet("allitem") - 1
            Dim idtrans As String = sqls.getDataSet("allitem", i, "idtransaksi")
            Dim sqls2 As New SQLs(dbstring)
            Dim field As New List(Of String)
            Dim value As New List(Of Object)
            field.AddRange(New String() {"@idtrans"})
            value.AddRange(New Object() {idtrans})
            sqls2.CallSP("spLoadLOVItemSPP", "getitem", field, value)
            Dim cek As Boolean = True
            For a As Integer = 0 To sqls2.getDataSet("getitem") - 1
                Dim qtyc As Long = 0
                Dim qtyt As Long = 0
                qtyt = sqls2.getDataSet("getitem", a, "qtyt")
                qtyc = sqls2.getDataSet("getitem", a, "qtyc")
                If qtyt < qtyc Then
                    cek = False
                End If
            Next
            If cek = True Then
                sqls2.DMLQuery("update transaksi set transaksistatus=8,updatedby='" & userid & "',updateddate=getdate(),updatedfromip='" & ipadd & "',updatedfromhostname='" & hostnm & "' where idtransaksi='" & idtrans & "'", False)
            End If
        Next
    End Sub
    ...
    Private Sub btnNew_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNew.Click
        ...
        For i As Integer = 0 To sqls.getDataSet("allitem") - 1
            Dim idtrans As String = sqls.getDataSet("allitem", 0, "idtransaksi")
            Dim sqls2 As New SQLs(dbstring)
            Dim field As New List(Of String)
            Dim value As New List(Of Object)
            field.AddRange(New String() {"@idtrans"})
            ...
            For a As Integer = 0 To sqls2.getDataSet("getitem") - 1
                Dim qtyc As Long = 0
                Dim qtyt As Long = 0
                qtyt = sqls2.getDataSet("getitem", 0, "qtyt")
                qtyc = sqls2.getDataSet("getitem", 0, "qtyc")
                If qtyt < qtyc Then
                    cek = False
                End If
            Next
            If cek = True Then
                sqls.DMLQuery("update transaksi set transaksistatus=8 where idtransaksi='" & idtrans & "'", False)
            End If
        Next
        ...
        If cari.ShowDialog = Windows.Forms.DialogResult.OK Then
            ...
            sqls.DMLQuery("select idasal from transaksi where idtransaksi='" & idParent & "'", "getidunit")
    ...
    Private Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If lueSupplier.EditValue Is Nothing Then
            bEntry = False
            If sEntry.Length = 0 Then
                sEntry = "Supplier belum dipilih"
            Else
                sEntry &= vbCrLf & "Supplier belum dipilih"
            End If
        End If
        ...
        If statData = statusData.Baru Then
            Dim pair As KeyValuePair(Of String, String) = generateno2(lueUnit.EditValue, iddeptpurc, "Pembelian Barang", False)
        ...
        sqls.DMLQuery("select top 1 convert(bigint,value) as value from sys_appsetting where variable='AllowAutoReviewPO'", "autoreview")
        ...
        Else
            field.AddRange(New String() {"idtransaksi", "idtransaksireff", "transaksitype", "transaksino", "transaksistatus", "idasal", "asaltype", "iddeptasal", "idtujuan", "tujuantype", "isdeleted", "remarks", "createdby", "createddate", "createdfromip", "createdfromhostname", "idcompany", "kirimdate", "tempodate", "reviewedby", "revieweddate", "reviewedfromip", "reviewedfromhostname"})
            value.AddRange(New Object() {idData, idParent, idtrans, teKode.Text, 6, lueUnit.EditValue, "Unit", iddeptpurc, lueSupplier.EditValue, "BusinessPartner", 0, teNote.Text, userid, nowTime, getIPAddress(ipaddparam.IP), getIPAddress(ipaddparam.Host), idcomp, If(deTanggalKirim.EditValue Is Nothing, DBNull.Value, CDate(deTanggalKirim.EditValue)), If(deTanggalTempo.EditValue Is Nothing, DBNull.Value, CDate(deTanggalTempo.EditValue)), userid, nowTime, getIPAddress(ipaddparam.IP), getIPAddress(ipaddparam.Host)})
        End If
    ...
    Private Sub btnSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        ...
        If cari.ShowDialog() = Windows.Forms.DialogResult.OK Then
            ...
            sqls.DMLQuery("select idtransaksireff,transaksino,convert(varchar,createddate,105) as tanggal,convert(varchar,createddate,108) as waktu,remarks,idasal,idtujuan,convert(varchar,kirimdate,105)+' '+convert(varchar,kirimdate,108) as kirimdate,convert(varchar,tempodate,105)+' '+convert(varchar,tempodate,108) as tempodate from transaksi where idtransaksi='" & idData & "'", "header")
            ...
            If sqls.getDataSet("header", 0, "kirimdate") IsNot Nothing Then
                ...
                deTanggalKirim.EditValue = Strdatetime2Datetime(tmptglkrm)
            End If
            If sqls.getDataSet("header", 0, "tempodate") IsNot Nothing Then
                ...
                deTanggalTempo.EditValue = Strdatetime2Datetime(tmptgltempo)
            End If
    ...
    Private Sub lueUnit_EditValueChanged(sender As Object, e As EventArgs) Handles lueUnit.EditValueChanged
        ...
        Else
            ...
            If sqls.getDataSet("getidpurc") > 0 Then
                ...
                Dim pair As KeyValuePair(Of String, String) = generateno2(lueUnit.EditValue, iddeptpurc, "Pembelian Barang", True)
                Dim idtrans As String = pair.Key
                Dim notrans As String = pair.Value
                teKode.Text = notrans
                Me.idtrans = idtrans
            ...
            For i As Integer = 0 To gvItem.RowCount - 1
                ...
                For a As Integer = 0 To gvData.RowCount - 1
                    ...
                    If drc("iditem") = dra("iditem") Then
                        cekExist = True
                        qtycharges = CInt(drc("qtycharges"))
                        idx = a
                        Exit For
                    End If
                Next
    ...
    Private Sub teSearch_KeyPress(sender As Object, e As KeyPressEventArgs) Handles teSearch.KeyPress
        ...
        If Asc(e.KeyChar) = 13 Then
            Dim sqls As New SQLs(dbstring)
            sqls.DMLQuery("select dt.iditem,i.item,convert(bit,0) as cek,isnull(qt.qtyc,0) as qtyc,isnull(qtt.qtyt,0) as qtyt,isnull(qt.qtyc,0)-isnull(qtt.qtyt,0) as qty,'Qty SPP ' + convert(varchar,convert(bigint,isnull(qt.qtyc,0))) + ' Qty PO ' + convert(varchar,convert(bigint,isnull(qtt.qtyt,0))) as note from transaksidt dt left join item i on dt.iditem=i.iditem left join (select iditem,sum(qtycharges) as qtyc from transaksidt where idtransaksi2=@idtrans group by iditem) qt on dt.iditem=qt.iditem left join (select dt.iditem,sum(dt.qtyreceive) as qtyt from transaksidt dt left join transaksi t on dt.idtransaksi=t.idtransaksi where t.transaksitype in ( select idtransactiontype from transactiontype where iddepartment in (select iddepartment from department where iddepartment in(select value from sys_appsetting where variable='IDPurcDept')) and idunit in (select idasal from transaksi where idtransaksi=@idtrans) and jenistransaksi='Pembelian Barang') group by dt.iditem) qtt on dt.iditem=qtt.iditem where dt.idtransaksi2='" & idParent & "' and (i.iditem='" & teSearch.Text & "' or i.kodeupc='" & teSearch.Text & "')", "getitem")
            ...
            If sqls.getDataSet("getitem") > 0 Then
                ...
                If cekExist = False Then
                    ...
                    dr("idtransaksidt") = "0"
                    dr("iditem") = iditem
                    sqls.DMLQuery("Select i.iditem,i.itemtype,i.idsatuan,gc.generalcode As type,i.kode,i.item,i.isppn,s.satuan,1 As qty from item i left join sys_generalcode gc On gc.idgeneral=i.itemtype And gc.gctype='ITEMTYPE' left join satuan s on s.idsatuan=i.idsatuan where i.iditem='" & iditem & "'", "dataitem")
                    dr("itemtype") = sqls.getDataSet("dataitem", 0, "itemtype")
                    dr("idsatuan") = sqls.getDataSet("dataitem", 0, "idsatuan")
                    dr("type") = sqls.getDataSet("dataitem", 0, "type")
                    dr("kode") = sqls.getDataSet("dataitem", 0, "kode")
    ...
    Private Sub Search_KeyDown(sender As Object, e As KeyEventArgs) Handles gvData.KeyDown, gcData.KeyDown, lueUnit.KeyDown, gvItem.KeyDown, gcItem.KeyDown, btnAllItem.KeyDown, btnDeleteAllItem.KeyDown, btnDeleteItem.KeyDown, btnItemSelect.KeyDown, btnNew.KeyDown, btnSave.KeyDown, tlpForm.KeyDown, pTitle.KeyDown, pBody.KeyDown, Panel1.KeyDown, tlpField.KeyDown, xtcItem.KeyDown, xtpItem.KeyDown, pItem.KeyDown, pHeader.KeyDown, pSearch.KeyDown, btnShowHideItem.KeyDown, lueSupplier.KeyDown, lueUnit.KeyDown, deTanggal.KeyDown, deTanggalKirim.KeyDown, deTanggalTempo.KeyDown, teKode.KeyDown, teNote.KeyDown
