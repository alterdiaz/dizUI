...
Public Class frmSuratJalanPermintaan
    ...
    Private Sub btnSearch_Click(sender As Object, e As EventArgs) Handles btnSearch.Click
        Dim sqls As New SQLs(dbstring)
        sqls.DMLQuery("select t.idtransaksi,t.transaksino,tf.transaksino as reffno,t.remarks,isnull(dt.counter,0) as totalitem,isnull(dtot.total,0) as totalbarang,convert(varchar,t.createddate,105)+' '+convert(varchar,t.createddate,108) as createddate,c.username from transaksi t left join transaksi tf on t.idtransaksireff=tf.idtransaksi left join sys_user c on c.iduser=t.createdby left join (select t.idtransaksi,isnull(count(dt.idtransaksidt),0) as counter from transaksi t left join transaksidt dt on t.idtransaksi=dt.idtransaksi group by t.idtransaksi ) dt on t.idtransaksireff=dt.idtransaksi left join (select t.idtransaksi,isnull(sum(dt.qtycharges),0) as total from transaksi t left join transaksidt dt on t.idtransaksi=dt.idtransaksi group by t.idtransaksi ) dtot on t.idtransaksireff=dtot.idtransaksi where t.transaksitype in (select tt.idtransactiontype from transactiontype tt where tt.kodetransaksi=(select [value] from sys_appsetting where [variable]='KodeSuratJalanPermintaan')) and t.transaksistatus in (1) and t.reviewedby is null and t.isdeleted=0 order by t.createddate desc", "search")
