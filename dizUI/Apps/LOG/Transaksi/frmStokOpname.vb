...
Public Class frmStokOpname
    ...
    Private Sub kosongkan()
        ...
        dePeriode.EditValue = New Date(nowTime.Year, nowTime.Month, 1, 0, 0, 0)
        lueUnit.EditValue = Nothing
        lueDept.EditValue = Nothing
    End Sub
    ...
    Private Sub loadgrid(idunit As String, iddept As String)
        Dim sqls As New SQLs(dbstring)
        sqls.DMLQuery("select ROW_NUMBER() OVER (Order by i.item asc) as nomor,i.iditem,ig.itemgrup,i.kode,i.item,isnull(iu.qty,0) as qty,isnull(iu.qty,0) as qtyinput from item i left join itemgrup ig on i.iditemgrup=ig.iditemgrup left join itemunit iu on iu.iditem=i.iditem and iu.idunit='" & idunit & "' and iu.iddepartment='" & iddept & "' where i.itemtype in (1,11) order by i.item asc", "data")
