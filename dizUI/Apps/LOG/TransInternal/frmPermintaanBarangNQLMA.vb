...
Public Class frmPermintaanBarangNQLMA
    Const HTCAPTION = &H2
    ...
    Private Sub frmPermintaanBarang_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If formTitle <> "" Then
            Me.Text = formTitle
    ...
    Private Sub save(iscetak As Boolean)
        ...
        If isautoreview = "0" Then
            ...
            End If
            value.AddRange(New Object() {idData, idtrans, teKode.Text, 1, lueUnit.EditValue, "Unit", iddept2(lueUnit.ItemIndex), 0, If(teNote.EditValue, "-"), userid, nowTime, getIPAddress(ipaddparam.IP), getIPAddress(ipaddparam.Host), idcomp})
        Else
            ...
            value.AddRange(New Object() {idData, idtrans, teKode.Text, 6, lueUnit.EditValue, "Unit", iddept2(lueUnit.ItemIndex), 0, If(teNote.EditValue, "-"), userid, nowTime, getIPAddress(ipaddparam.IP), getIPAddress(ipaddparam.Host), userid, nowTime, getIPAddress(ipaddparam.IP), getIPAddress(ipaddparam.Host), idcomp})
        End If
        ...
        If iscetak = True Then
            ...
            Dim sharename As String = getPrinter("SPBLMA")
            If sharename <> "" Then
                Try
                    pt.Print(sharename)
    ...
    Private Sub btnItemUnit_Click(sender As Object, e As EventArgs) Handles btnItemUnit.Click
        ...
        sqls2.DMLQuery("select ia.iditem,ia.qtyask from itemunitask ia where ia.idunit='" & idunit2(lueUnit.ItemIndex) & "' and ia.iddepartment='" & iddept2(lueUnit.ItemIndex) & "' and replace(convert(varchar,ia.askdate,105),'-','') like '%" & Format(CDate(deTanggal.EditValue), "MMyyyy") & "'", "cekdata")
        If sqls2.getDataSet("cekdata") = 0 Then
            sqls2.DMLQuery("select ia.iditem,ia.qtyask from itemunitask ia where ia.idunit='" & idunit2(lueUnit.ItemIndex) & "' and ia.iddepartment='0' and replace(convert(varchar,ia.askdate,105),'-','') like '%" & Format(CDate(deTanggal.EditValue), "MMyyyy") & "'", "cekdata2")
