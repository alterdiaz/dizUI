Public Class frmBukuBesar
    ...
    Private Sub deTanggal_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles deTanggal.EditValueChanged, slueCOA.EditValueChanged
        ...
        If sqls.getDataSet("gettipecoa", 0, "tipecoa") = "1" Then
            Dim tmparr() As String = IO.File.ReadAllLines(appPath & "BukuBesarDK.txt")
        ...
        ElseIf sqls.getDataSet("gettipecoa", 0, "tipecoa") = "2" Then
            Dim tmparr() As String = IO.File.ReadAllLines(appPath & "BukuBesarKD.txt")
            For i As Integer = 0 To tmparr.Length - 1
                strQuery &= tmparr(i)
                If tmparr(i).Length > 0 Then
                    If tmparr(i).Chars(tmparr(i).Length - 1) <> " " Then
        ...
        str = str.Replace("*COA*", slueCOA.Text)
        ...
        If sqls.getDataSet("gettipecoa", 0, "tipecoa") = "0" Then
            gcData.DataSource = Nothing
        Else
        ...
        sqls.DMLQuery("select remarks from coa where idcoa='" & slueCOA.EditValue & "'", "rem")
    ...
    Private Sub frmBukuBesar_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        mysqls.DMLQuery("select idcoa, coa, remarks, isnull(tipecoa,0) as tipecoa from coa where idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and coa in (select distinct(left(c.coa,5)) as coa from coa c inner join jurnal j on j.idcoa=c.idcoa and j.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') where c.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID')) and len(convert(decimal(20,0),COA))=5 and (isnull(isdeleted,0)=0 or convert(decimal(20,0),COA)<>-1) order by convert(varchar(20),COA) asc", "coa")
