...
Public Class frmPenerimaanBarang
    ...
    Private Sub frmPenerimaanBarangM_Load(sender As Object, e As EventArgs) Handles Me.Load
        If formTitle <> "" Then
            Me.Text = formTitle
    ...
    Private Sub btnItemSelect_Click(sender As Object, e As EventArgs) Handles btnItemSelect.Click
        ...
        Dim sqls As New SQLs(dbstring)
        sqls.DMLQuery("select d.idtransaksidt,d.iditem,i.itemtype,it.generalcode as type,i.kode,i.item,d.qtycontent,d.qtycharges as qtypo,i.idsatuan,d.idsatuan as idsatuanpo,d.idsatuan2,si.satuan,sp.satuan as satuanpo,ss.satuan as satuan2,isnull(d2.qtycharges,d.qtycharges) as qtycharges,d.remarks,isnull(d2.qtycharges,d.qtycharges)*d.qtycontent as qty from transaksidt d left join transaksidt d2 on d.idtransaksi=d2.idtransaksi2 left join item i on d.iditem=i.iditem left join sys_generalcode it on i.itemtype=it.idgeneral and it.gctype='itemtype' left join satuan sp on d.idsatuan=sp.idsatuan left join satuan ss on d.idsatuan2=ss.idsatuan left join satuan si on i.idsatuan=si.idsatuan where d.idtransaksi='" & idpo & "'", "cekpo")
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        Else
            ...
            For i As Integer = 0 To gvData.RowCount - 1
                ...
                valuelog.AddRange(New Object() {iditemlog, iditem, iddept, lueUnit.EditValue, iddata, "Transaksi", iddatadt, "TransaksiDT", teKode.Text, "Penerimaan Barang", qty, 1, "Penerimaan Barang " & teKode.Text, userid, nowTime})
        ...
        fielddt.AddRange(New String() {"idtransaksidt", "idtransaksi2", "iditem", "kodeitem", "item", "itemtype", "type", "qtydispose", "qtycharges", "qtyreceive", "idsatuan", "satuan", "qtycontent", "idsatuan2", "satuan2", "harga", "subtotal", "discpersen", "discamount", "discsubtotal", "ppn", "remarks", "isdeleted", "createdby", "createddate", "createdfromip", "createdfromhostname", "idcompany"})
            For i As Integer = 0 To gvData.RowCount - 1
                ...
                If iddt = "0" Then
                    iddt = GenerateGUID()
                End If
                valuedt.AddRange(New Object() {iddt, iddata, dr("iditem"), dr("kode"), dr("item"), dr("itemtype"), dr("type"), dr("qtycharges"), dr("qtycharges"), dr("qty"), dr("idsatuan"), dr("satuan"), dr("qtycontent"), dr("idsatuan2"), dr("satuan2"), 0, 0, 0, 0, 0, 0, dr("remarks"), 0, userid, nowTime, getIPAddress(ipaddparam.IP), getIPAddress(ipaddparam.Host), idcomp})
