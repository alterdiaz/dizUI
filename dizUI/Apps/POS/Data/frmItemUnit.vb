Public Class frmItemUnit
    ...
    Private Sub loadGrid()
        ...
        mysqls.DMLQuery("select iu.iditemunit,iu.iditem,i.kode,i.item,iu.qty,iu.idunit,u.unit,iu.iddepartment,d.department,s.satuan from itemunit iu left join item i on iu.iditem=i.iditem left join unit u on iu.idunit=u.idunit left join department d on d.iddepartment=iu.iddepartment left join satuan s on i.idsatuan=s.idsatuan where year(iu.createddate)='" & nowTime.Year & "' order by u.unit asc,d.department asc,i.item asc", "data")
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If statData = statusData.Baru Then
            idData = GenerateGUID()
            field.AddRange(New String() {"iditemunit", "iditem", "iddepartment", "idunit", "qty", "createdby", "createddate"})
            value.AddRange(New Object() {idData, lueItem.EditValue, lueDept.EditValue, lueUnit.EditValue, seQty.Value, userid, New Date(nowTime.Year, 1, 1, 1, 1, 1)})
            ...
            fieldLog.AddRange(New String() {"iditemlog", "iditem", "iddepartment", "idunit", "idreff", "idreff2", "noreff", "trfreff", "qty", "io", "remarks", "createdby", "createddate"})
            valueLog.AddRange(New Object() {iditemlog, lueItem.EditValue, lueDept.EditValue, lueUnit.EditValue, idData, 0, 0, "SaldoAwal " & nowTime.Year, seQty.Value, "in", "Saldo Awal " & nowTime.Year, userid, New Date(nowTime.Year, 1, 1, 1, 1, 1)})
        Else
            field.AddRange(New String() {"iditemunit", "qty", "updatedby", "updateddate"})
            value.AddRange(New Object() {idData, seQty.Value, userid, nowTime})
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        ...
        If btnDelete.Text = "HAPUS" Then
            field.AddRange(New String() {"iditemunit", "isdeleted", "updatedby", "updateddate"})
            value.AddRange(New Object() {idData, 1, userid, nowTime})
            retval = sqls.datasetSave("itemunit", idData, field, value, False)
            If retval Then
