Public Class frmPelayanan
    ...
    Private Sub loadGrid()
        ...
        mysqls.DMLQuery("select dm.idpelayanan,dm.pelayanan,dm.isdeleted,del.generalcode as statdata,dm.createdby, convert(varchar,dm.createddate,105) as createddate from pelayanan dm left join sys_generalcode del on dm.isdeleted=del.idgeneral and del.gctype='DELETE' order by dm.pelayanan asc", "data")
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If statData = statusData.Baru Then
            ...
            sqls.DMLQuery("select pelayanan from pelayanan where replace(pelayanan,' ','')='" & teNama.Text.Replace(" ", "") & "'", "exist")
        ...
        ElseIf statData = statusData.Edit Then
            Dim sqls As New SQLs(dbstring)
            sqls.DMLQuery("select pelayanan from pelayanan where replace(pelayanan,' ','')='" & teNama.Text.Replace(" ", "") & "' and idpelayanan<>" & idData, "exist")
            If sqls.getDataSet("exist") > 0 Then
                dizMsgbox("Data tersebut sudah ada", dizMsgboxStyle.Info, Me)
                teNama.Focus()
                Exit Sub
        ...
        If statData = statusData.Baru Then
            idData = GenerateGUID()
            field.AddRange(New String() {"idpelayanan", "pelayanan", "isdeleted", "createdby", "createddate"})
            value.AddRange(New Object() {idData, teNama.Text, 0, userid, nowTime})
        Else
            field.AddRange(New String() {"idpelayanan", "pelayanan", "updatedby", "updateddate"})
            value.AddRange(New Object() {idData, teNama.Text, userid, nowTime})
        End If

        If dtSQL.datasetSave("pelayanan", idData, field, value, False) = True Then
            dizMsgbox("Data tersimpan", dizMsgboxStyle.Info, Me)
            isiLog(userid, dbstring, field, value, "pelayanan")
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        ...
        If btnDelete.Text = "HAPUS" Then
            field.AddRange(New String() {"idpelayanan", "isdeleted", "updatedby", "updateddate"})
            value.AddRange(New Object() {idData, 1, userid, nowTime})
            retval = sqls.datasetSave("pelayanan", idData, field, value, False)
            If retval Then
    ...
    Private Sub gvData_FocusedRowChanged(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs) Handles gvData.FocusedRowChanged
        Try
            ...
            teNama.Text = dcol("pelayanan")

            idData = dcol("idpelayanan")
    ...
    Private Sub Form_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        checkFieldMaxLength(dbstring, tlpField, "pelayanan")
        btnNew_Click(Me, Nothing)
    End Sub
