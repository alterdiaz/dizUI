Public Class frmICD
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If statData = statusData.Baru Then
            ...
            sqls.DMLQuery("select diagnosa from ICD where idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and replace(diagnosa,' ','')='" & teNama.Text.Replace(" ", "") & "' and icdtype='" & lueType.EditValue & "'", "exist")
            ...
            sqls.DMLQuery("select kode from ICD where idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and replace(kode,' ','')='" & teKode.Text.Replace(" ", "") & "' and icdtype='" & lueType.EditValue & "'", "exist2")
        ...
        ElseIf statData = statusData.Edit Then
            Dim sqls As New SQLs(dbstring)
            sqls.DMLQuery("select diagnosa from ICD where idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and replace(diagnosa,' ','')='" & teNama.Text.Replace(" ", "") & "' and icdtype='" & lueType.EditValue & "' and idicd<>'" & idData & "'", "exist")
            If sqls.getDataSet("exist") > 0 Then
                dizMsgbox("Data tersebut sudah ada", dizMsgboxStyle.Info, Me)
                teNama.Focus()
                Exit Sub
            End If
            sqls.DMLQuery("select kode from ICD where idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and replace(kode,' ','')='" & teKode.Text.Replace(" ", "") & "' and icdtype='" & lueType.EditValue & "' and idicd<>'" & idData, "exist2")
            If sqls.getDataSet("exist2") > 0 Then
                dizMsgbox("Data tersebut sudah ada", dizMsgboxStyle.Info, Me)
                teNama.Focus()
                Exit Sub
