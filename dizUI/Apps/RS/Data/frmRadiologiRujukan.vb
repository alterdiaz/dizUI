Public Class frmRadiologiRujukan
    Const HTCAPTION = &H2
    ...
    Private Sub loadLOV()
        Dim mysqls As New SQLs(dbstring)
        mysqls.DMLQuery("select idgeneral as id,generalcode as content from sys_generalcode where generalcode='RADIOLOGI' and gctype='BPTYPE'", "type")
    ...
    Private Sub btnSaveEmail_Click(sender As Object, e As EventArgs) Handles btnSaveEmail.Click
        ...
        If statDataChild = statusData.Baru Then
            Dim sqls As New SQLs(dbstring)
            sqls.DMLQuery("select email from email where idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and tablereff='BP' and idreff='" & idData & "' and email='" & teEmail.Text & "'", "cek")
    ...
    Private Sub btnSaveTelepon_Click(sender As Object, e As EventArgs) Handles btnSaveTelepon.Click
        ...
        If statDataChild = statusData.Baru Then
            Dim sqls As New SQLs(dbstring)
            sqls.DMLQuery("select phone from phone where idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and tablereff='BP' and idreff='" & idData & "' and phone='" & tePhone.Text & "'", "cek")
            If sqls.getDataSet("cek") = 0 Then
                idDataChild = GenerateGUID()
            Else
