Public Class frmSpesialis
    ...
    Private Sub loadGrid()
        ...
        mysqls.DMLQuery("select s.idspesialis,s.nama,s.remarks,s.isdeleted,del.generalcode as statdata from spesialis s left join sys_generalcode del on s.isdeleted=del.idgeneral and del.gctype='DELETE' order by s.nama asc", "data")
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If statData = statusData.Baru Then
            ...
            sqls.DMLQuery("select nama from spesialis where replace(nama,' ','')='" & teNama.Text.Replace(" ", "") & "'", "exist")
        ...
        ElseIf statData = statusData.Edit Then
            Dim sqls As New SQLs(dbstring)
            sqls.DMLQuery("select nama from spesialis where replace(nama,' ','')='" & teNama.Text.Replace(" ", "") & "' and idspesialis<>" & idData, "exist")
            If sqls.getDataSet("exist") > 0 Then
                dizMsgbox("Data tersebut sudah ada", dizMsgboxStyle.Info, Me)
                teNama.Focus()
                Exit Sub
        ...
        If statData = statusData.Baru Then
            field.AddRange(New String() {"idspesialis", "nama", "remarks", "isdeleted", "createdby", "createddate", "idcompany"})
            value.AddRange(New Object() {idData, teNama.Text, teRemarks.Text, 0, userid, nowTime, idcomp})
        Else
            field.AddRange(New String() {"idspesialis", "nama", "remarks", "updatedby", "updateddate"})
            value.AddRange(New Object() {idData, teNama.Text, teRemarks.Text, userid, nowTime})
        End If

        If dtSQL.datasetSave("spesialis", idData, field, value, False) = True Then
            dizMsgbox("Data tersimpan", dizMsgboxStyle.Info, Me)
            isiLog(userid, dbstring, field, value, "spesialis")
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        ...
        If btnDelete.Text = "HAPUS" Then
            field.AddRange(New String() {"idspesialis", "isdeleted", "updatedby", "updateddate"})
            value.AddRange(New Object() {idData, 1, userid, nowTime})
            retval = sqls.datasetSave("spesialis", idData, field, value, False)
            If retval Then
    ...
    Private Sub Form_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        checkFieldMaxLength(dbstring, tlpField, "spesialis")
        btnNew_Click(Me, Nothing)
    End Sub
