Public Class frmDietMenu
    ...
    Private Sub loadGrid()
        ...
        mysqls.DMLQuery("select dm.iddietmenu,dm.diettype,gcd.generalcode as jenisdiet,dm.dietmenu,dm.remarks,dm.isdeleted,del.generalcode as statdata,dm.createdby, convert(varchar,dm.createddate,105) as createddate  from dietmenu dm left join sys_generalcode gcd on dm.diettype=gcd.idgeneral and gcd.gctype='DIETMENU' left join sys_generalcode del on dm.isdeleted=del.idgeneral and del.gctype='DELETE' where dm.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') order by gcd.generalcode asc,dm.dietmenu asc", "data")
    ...
    Private Sub loadLOV()
        Dim mysqls As New SQLs(dbstring)
        mysqls.DMLQuery("select idgeneral as id,generalcode as content from sys_generalcode where gctype='DIETMENU'", "type")
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If statData = statusData.Baru Then
            ...
            sqls.DMLQuery("select dietmenu from dietmenu where idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and replace(dietmenu,' ','')='" & teNama.Text.Replace(" ", "") & "' and diettype='" & lueType.EditValue & "'", "exist")
        ...
        ElseIf statData = statusData.Edit Then
            Dim sqls As New SQLs(dbstring)
            sqls.DMLQuery("select dietmenu from dietmenu where idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and replace(dietmenu,' ','')='" & teNama.Text.Replace(" ", "") & "' and diettype='" & lueType.EditValue & "' and iddietmenu<>'" & idData & "'", "exist")
            If sqls.getDataSet("exist") > 0 Then
                dizMsgbox("Data tersebut sudah ada", dizMsgboxStyle.Info, Me)
                teNama.Focus()
                Exit Sub
        ...
        If statData = statusData.Baru Then
            idData = GenerateGUID()
            field.AddRange(New String() {"iddietmenu", "idcompany", "diettype", "dietmenu", "remarks", "isdeleted", "createdby", "createddate"})
            value.AddRange(New Object() {idData, idcomp, lueType.EditValue, teNama.Text, teRemarks.Text, 0, userid, nowTime})
        Else
            field.AddRange(New String() {"iddietmenu", "diettype", "dietmenu", "remarks", "updatedby", "updateddate"})
            value.AddRange(New Object() {idData, lueType.EditValue, teNama.Text, teRemarks.Text, userid, nowTime})
        End If

        If dtSQL.datasetSave("dietmenu", idData, field, value, False) = True Then
            dizMsgbox("Data tersimpan", dizMsgboxStyle.Info, Me)
            isiLog(userid, dbstring, field, value, "dietmenu")
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        ...
        If btnDelete.Text = "HAPUS" Then
            field.AddRange(New String() {"iddietmenu", "isdeleted", "updatedby", "updateddate"})
            value.AddRange(New Object() {idData, 1, userid, nowTime})
            retval = sqls.datasetSave("dietmenu", idData, field, value, False)
            If retval Then
    ...
    Private Sub gvData_FocusedRowChanged(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs) Handles gvData.FocusedRowChanged
        Try
            ...
            lueType.EditValue = dcol("diettype")

            teNama.Text = dcol("dietmenu")
    ...
    Private Sub Form_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        checkFieldMaxLength(dbstring, tlpField, "dietmenu")
        btnNew_Click(Me, Nothing)
    End Sub
