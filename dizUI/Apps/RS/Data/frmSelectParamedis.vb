        ...
        If justID = False Then
            If idspesialis(0) = "*" Then
                If idspesialisexcept.Count = 0 Then
                    mysqls.DMLQuery("select p.idparamedis,p.nama,isnull(s.nama,'-') as spesialis from Paramedis p left join ParamedisSpesialis ps on p.idparamedis=ps.idparamedis left join Spesialis s on ps.idspesialis=s.idspesialis where p.isdeleted=0 and p.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') order by s.nama,p.nama asc", "param")
                Else
            ...
            Else
                If idspesialis.Count > 1 Then
                    If idspesialisexcept.Count = 0 Then
                        mysqls.DMLQuery("select p.idparamedis,p.nama,isnull(s.nama,'-') as spesialis from Paramedis p left join ParamedisSpesialis ps on p.idparamedis=ps.idparamedis left join Spesialis s on ps.idspesialis=s.idspesialis where p.isdeleted=0 and s.idspesialis in (" & String.Join(",", idspesialis) & ") and p.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') order by s.nama,p.nama asc", "param")
                    Else
            ...
            End If
        Else
            mysqls.DMLQuery("select p.nama,isnull(s.nama,'-') as spesialis from Paramedis p left join ParamedisSpesialis ps on p.idparamedis=ps.idparamedis left join Spesialis s on ps.idspesialis=s.idspesialis where p.isdeleted=0 and p.idparamedis in (" & String.Join(",", idselect) & ") and p.idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') order by s.nama,p.nama asc", "param")
        End If
