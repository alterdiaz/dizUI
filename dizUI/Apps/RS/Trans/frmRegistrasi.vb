Public Class frmRegistrasi
    ...
    Private Sub btnNew_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNew.Click
        ...
        Else
            ...
            If selectPelayanan.getStringPilih = "KONSULTASI" Then
                ...
                sqls.DMLQuery("select idlokasi as id,nama as content from lokasi where lokasitype=3 and iddepartment in (select [value] from sys_appsetting where variable='IDOutpatientDept') order by nama asc", "jadwallokasi")
            ...
            ElseIf selectPelayanan.getStringPilih = "REHABILITASI" Then
                ...
                sqls.DMLQuery("select idlokasi as id,nama as content from lokasi where lokasitype=27 and iddepartment in (select [value] from sys_appsetting where variable='IDPhysioDept') order by nama asc", "jadwallokasi")
                lueRuang.Properties.DataSource = sqls.dataTable("jadwallokasi")
                lueRuang.Properties.DisplayMember = "content"
                lueRuang.Properties.ValueMember = "id"
                lueRuang.Properties.BestFitMode = DevExpress.XtraEditors.Controls.BestFitMode.BestFitResizePopup
            ...
            ElseIf selectPelayanan.getStringPilih = "UMUM" Then
                ...
                sqls.DMLQuery("select idlokasi as id,nama as content from lokasi where lokasitype in (3) and iddepartment in (select [value] from sys_appsetting where variable='IDOutpatientDept') order by nama asc", "jadwallokasi")
                lueRuang.Properties.DataSource = sqls.dataTable("jadwallokasi")
                lueRuang.Properties.DisplayMember = "content"
                lueRuang.Properties.ValueMember = "id"
                lueRuang.Properties.BestFitMode = DevExpress.XtraEditors.Controls.BestFitMode.BestFitResizePopup
    ...
    Private Sub SearchRM_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles lnkNoRM.LinkClicked, lnkNama.LinkClicked, lnkTanggalLahir.LinkClicked
        ...
        For i As Integer = 0 To tmparr.Length - 1
            ...
            End If
        Next
        Dim strcek As String = " where isdeleted=0 and idcompany=(select top 1 value from sys_appsetting where variable='CompanyID')"
    ...
    Private Sub lueJenisPembayar_EditValueChanged(sender As Object, e As EventArgs) Handles lueJenisPembayar.EditValueChanged
        ...
        Else
            If lueJenisPembayar.EditValue = 1 Then
                ...
                End If
                luePayer.EditValue = Nothing
            ElseIf lueJenisPembayar.EditValue = 2 Then
                lblAsuransi.Visible = True
                lblAsuransi.Text = "Perusahaan"
                lblNoAsuransi.Visible = False
                teNoAsuransi.Visible = False
                ...
                luePayer.Properties.DataSource = Nothing
                luePayer.EditValue = 0
            ElseIf lueJenisPembayar.EditValue = 3 Then
                lblAsuransi.Visible = True
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        Dim sqls As New SQLs(dbstring)
        sqls.DMLQuery("select count(idregistrasi) as count from registrasi where isdeleted=0 and idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and idrekammedis='" & teNoRM.Tag & "'", "cnt")
        cnt = sqls.getDataSet("cnt", 0, "count")

        If iddept <> "0" Then
            sqls.DMLQuery("select count(idregistrasi) as count from registrasi where isdeleted=0 and idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and iddepartment='" & iddept & "' and idunit='" & idunit & "' and idrekammedis='" & teNoRM.Tag & "'", "cntdept")
            cntdept = sqls.getDataSet("cntdept", 0, "count")
        End If
        If lueParamedis.EditValue IsNot Nothing Then
            sqls.DMLQuery("select count(idregistrasi) as count from registrasi where isdeleted=0 and idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and iddokterruangan='" & lueParamedis.EditValue & "' and idrekammedis='" & teNoRM.Tag & "'", "cntdokter")
        ...
        Else
            ...
            value.AddRange(New Object() {idData, idtrans, If(lueSesi.EditValue, 0), idcomp, teNoRegistrasiInduk.Tag, iddept, idunit, idtrans, teNoRM.Tag, lueRuang.EditValue, idkelas, lueParamedis.EditValue, If(lueSpesialis.EditValue, 0), teNoRegistrasi.Text, cnt + 1, cntdokter + 1, cntdept + 1, If(lueAsalPx.EditValue, 0), If(lueAsalPx2.EditValue, 0), If(lueAsalPx3.EditValue, 0), If(lueAsalPx4.EditValue, 0), CLng(teJenisKunjungan.Tag), 0, remarks, lueJenisPembayar.EditValue, If(luePayer.EditValue, 0), If(lueAsuransi.EditValue, 0), lueHamil.EditValue, 0, "-", userid, If(lueODS.EditValue, 0)})
        End If
        ...
        If cek = True Then
            If teNoAsuransi.Visible = True Then
                ...
                sqls.DMLQuery("select idkartu from kartu where idreff='" & teNoRM.Tag & "' and tablereff='PASIEN' and nomorkartu='" & teNoAsuransi.Text & "'", "cekkartu")
    ...
    Private Sub lueParamedis_EditValueChanged(sender As Object, e As EventArgs) Handles lueParamedis.EditValueChanged
        ...
        If jenisPelayanan = "IGD" Or jenisPelayanan = "IGD ELEKTIF" Then
            sqls.DMLQuery("select idlokasi as id,nama as content from lokasi where nama = (select case when variable='IDEmergencyDept' then 'IGD' end as 'department' from sys_appsetting where variable in ('IDEmergencyDept'))", "jadwallokasi")
            lueRuang.Properties.DataSource = sqls.dataTable("jadwallokasi")
            lueRuang.Properties.DisplayMember = "content"
            lueRuang.Properties.ValueMember = "id"
            lueRuang.Properties.BestFitMode = DevExpress.XtraEditors.Controls.BestFitMode.BestFitResizePopup
    ...
    Private Sub btnSavePrint_Click(sender As Object, e As EventArgs) Handles btnSavePrint.Click
        ...
        Dim sqls As New SQLs(dbstring)
        sqls.DMLQuery("select count(idregistrasi) as count from registrasi where registrasistatus<>1 and idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and idrekammedis='" & teNoRM.Tag & "'", "cnt")
        If iddept <> "0" Then
            sqls.DMLQuery("select count(idregistrasi) as count from registrasi where registrasistatus<>1 and idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and iddepartment='" & iddept & "' and idunit='" & idunit & "' and idrekammedis='" & teNoRM.Tag & "'", "cntdept")
            cntdept = sqls.getDataSet("cntdept", 0, "count")
        End If
        If lueParamedis.EditValue IsNot Nothing Then
            sqls.DMLQuery("select count(idregistrasi) as count from registrasi where registrasistatus<>1 and idcompany=(select top 1 value from sys_appsetting where variable='CompanyID') and iddokterruangan='" & lueParamedis.EditValue & "' and idrekammedis='" & teNoRM.Tag & "'", "cntdokter")
            cntdokter = sqls.getDataSet("cntdokter", 0, "count")
        End If
        ...
        idData = GenerateGUID()
        If teNoRegistrasiInduk.Tag = "" Then
            field.AddRange(New String() {"idregistrasi", "idcompany", "iddepartment", "idunit", "transactiontype", "idrekammedis", "idlokasi", "idkelas", "iddokterruangan", "idspesialisruangan", "registrasino", "kunjunganke", "kunjungandokterke", "kunjungandepartmentke", "asalpx", "asalpx2", "asalrujukan", "iddokterrujukan", "jeniskunjungan", "registrasistatus", "remarks", "payertype", "idpayer", "idasuransi", "ishamil", "isdeleted", "deletereason", "createdby"})
            value.AddRange(New Object() {idData, idcomp, iddept, idunit, idtrans, CStr(teNoRM.Tag), lueRuang.EditValue, idkelas, lueParamedis.EditValue, If(lueSpesialis.EditValue, 0), teNoRegistrasi.Text, cnt + 1, cntdokter + 1, cntdept + 1, If(lueAsalPx.EditValue, 0), If(lueAsalPx2.EditValue, 0), If(lueAsalPx3.EditValue, 0), If(lueAsalPx4.EditValue, 0), CLng(teJenisKunjungan.Tag), 0, remarks, lueJenisPembayar.EditValue, If(luePayer.EditValue, 0), If(lueAsuransi.EditValue, 0), lueHamil.EditValue, 0, "-", userid})
        Else
            field.AddRange(New String() {"idregistrasi", "idcompany", "idregistrasiparent", "iddepartment", "idunit", "transactiontype", "idrekammedis", "idlokasi", "idkelas", "iddokterruangan", "idspesialisruangan", "registrasino", "kunjunganke", "kunjungandokterke", "kunjungandepartmentke", "asalpx", "asalpx2", "asalrujukan", "iddokterrujukan", "jeniskunjungan", "registrasistatus", "remarks", "payertype", "idpayer", "idasuransi", "ishamil", "isdeleted", "deletereason", "createdby"})
            value.AddRange(New Object() {idData, idcomp, CStr(teNoRegistrasiInduk.Tag), iddept, idunit, idtrans, CStr(teNoRM.Tag), lueRuang.EditValue, idkelas, lueParamedis.EditValue, If(lueSpesialis.EditValue, 0), teNoRegistrasi.Text, cnt + 1, cntdokter + 1, cntdept + 1, If(lueAsalPx.EditValue, 0), If(lueAsalPx2.EditValue, 0), If(lueAsalPx3.EditValue, 0), If(lueAsalPx4.EditValue, 0), CLng(teJenisKunjungan.Tag), 0, remarks, lueJenisPembayar.EditValue, If(luePayer.EditValue, 0), If(lueAsuransi.EditValue, 0), lueHamil.EditValue, 0, "-", userid})
        End If
        Dim dtsetsqls As New dtsetSQLS(dbstring)
        Dim cek As Boolean = dtsetsqls.datasetSave("registrasi", idData, field, value, False)
        If cek = True Then
            If teNoAsuransi.Visible = True Then
                ...
                If sqls.getDataSet("cekkartu") = 0 Then
                    ...
                    Dim fkartu As New List(Of String)
                    Dim vkartu As New List(Of Object)
                    fkartu.AddRange(New String() {"idkartu", "idreff", "tablereff", "idpenerbit", "jenispenerbit", "jeniskartu", "nomorkartu", "namakartu", "jenistanggal", "tanggalkartu", "isdeleted", "createdby"})
                    vkartu.AddRange(New Object() {idtmp, CStr(teNoRM.Tag), "PASIEN", lueAsuransi.EditValue, "ASURANSI", 11, teNoAsuransi.Text, teNama.Text, 3, nowTime, 0, userid})
                    dtsetsqls = New dtsetSQLS(dbstring)
                    dtsetsqls.datasetSave("kartu", idtmp, fkartu, vkartu, False)
                End If
            End If
    ...
    Private Sub btnCetakGelang_Click(sender As Object, e As EventArgs) Handles btnCetakGelang.Click
        ...
        If cari.ShowDialog = Windows.Forms.DialogResult.OK Then
            ...
            Dim idregu As String = sqls.getDataSet("getid", 0, "idregistrasi")
            ...
            Dim idrmu As String = sqls.getDataSet("getid", 0, "idrekammedis")
    ...
    Private Sub btnMonRegister_Click(sender As Object, e As EventArgs) Handles btnMonRegister.Click
        ...
        Dim frmMon As New frmMonRegisterAll
        tambahChild(frmMon)
        frmMon.Size = New Size(Screen.PrimaryScreen.WorkingArea.Width - 100, Screen.PrimaryScreen.WorkingArea.Height - 100)
        frmMon.MaximumSize = New Size(Screen.PrimaryScreen.WorkingArea.Width - 100, Screen.PrimaryScreen.WorkingArea.Height - 100)
        frmMon.StartPosition = FormStartPosition.CenterScreen
    ...
    Private Sub btnMonRegHarian_Click(sender As Object, e As EventArgs) Handles btnMonRegHarian.Click
        ...
        Dim frmMon As New frmMonRegisterAllDay
        tambahChild(frmMon)
        frmMon.Size = New Size(Screen.PrimaryScreen.WorkingArea.Width - 100, Screen.PrimaryScreen.WorkingArea.Height - 100)
        frmMon.MaximumSize = New Size(Screen.PrimaryScreen.WorkingArea.Width - 100, Screen.PrimaryScreen.WorkingArea.Height - 100)
        frmMon.StartPosition = FormStartPosition.CenterScreen
