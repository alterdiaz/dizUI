Public Class frmCurrentPayment
    ...
    Private Sub Me_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If sqli.getDataSet("prod") > 0 Then
            ...
            sqli.DMLQuery("select idproducttype from companyproducttype where idcompany='" & tmpidcompany & "'", "protype")
        ...
        If teNamaProduk.Text = "" Then
            dizMsgbox("Data Produk tidak ditemukan", dizMsgboxStyle.Peringatan, Me)
            Me.Dispose()
        End If
        ...
        If json_result.Length > 2 Then 'table.Rows.Count > 0 Then 'sqls.getDataSet("existhw") > 0 Then 'strvalue.Contains("ERROR") = False Then 
            table = Newtonsoft.Json.JsonConvert.DeserializeObject(Of DataTable)(json_result)
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        fs.AddRange(New String() {"idpayment", "idvoucher", "idbankoption", "idcompanyproduct", "idcompany", "idpaymentoption", "paymentno", "tanggalbayar", "nominalbayar", "nominaldata", "bankpengirim", "norekpengirim", "atasnamapengirim", "remarks", "isvalid", "isdeleted", "deletereason", "createdby", "createddate"})
        vs.AddRange(New String() {idpaymentonline, "0", lueRekening.EditValue, "0", idcompany, luePaket.EditValue, tePaymentNo.Text, Format(CDate(deTanggal.EditValue), "yyyy-MM-dd"), CStr(CLng(seNominal.EditValue)), tePaket.Text, teBank.Text, teNorek.Text, teNama.Text, meRemarks.Text, 0, 0, "-", tmpUserID, Format(CDate(nowTime), "yyyy-MM-dd HH:mm:ss")})
        ...
        If json_result = "true" Then 'table.Rows.Count > 0 Then 'dtsqls.datasetSave("payment", idtmp, fs, vs, False) = True Then
            ...
            ' idtmp = GenerateGUID()
            flite.AddRange(New String() {"idpayment", "idbankoption", "idcompanyproduct", "idcompany", "idpaymentoption", "paymentno", "tanggalbayar", "nominalbayar", "nominaldata", "bankpengirim", "norekpengirim", "atasnamapengirim", "remarks", "isvalid", "isdeleted", "deletereason", "createddate", "createdby"})
            vlite.AddRange(New Object() {idpaymentonline, lueRekening.EditValue, "0", idcompany, luePaket.EditValue, tePaymentNo.Text, CDate(deTanggal.EditValue), tePaket.Tag, tePaket.Text, teBank.Text, teNorek.Text, teNama.Text, meRemarks.Text, 0, 0, "", Format(tmpnowTime, "yyyy-MM-dd"), tmpUserID})
    ...
    Private Sub btnExistCheck_Click(sender As Object, e As EventArgs) Handles btnExistCheck.Click
        ...
        If json_result.Length = 2 Then 'table.Rows.Count = 0 Then 'sqls.getDataSet("cekvoucher") = 0 Then
        ...
        Else
            ...
            If json_result.Length > 2 Then 'table.Rows.Count > 0 Then  'sqls.getDataSet("cekvoucherused") > 0 Then
                table = Newtonsoft.Json.JsonConvert.DeserializeObject(Of DataTable)(json_result)
        ...
        If json_result = "true" Then 'table.Rows.Count > 0 Then 'dtsqls.datasetSave("payment", idpaymentonline, fs, vs, False) = True Then
            ...
            vlite.AddRange(New Object() {idpaymentonline, vid, "29B5A27C-1716-11E9-B068-28924AD3C84E", tmpidcompany, "0", "", tePaymentNo.Text, Format(tmpnowTime, "yyyy-MM-dd"), vnominal, vnominaldata, teBank.Text, teNorek.Text, teNama.Text, meRemarks.Text, 1, 0, "", Format(tmpnowTime, "yyyy-MM-dd HH:mm:ss"), tmpUserID})
            Dim dtlite As New dtsetSQLI(dblite)
            dtlite.datasetSave("payment", idpaymentonline, flite, vlite, False)
    ...
    '    If json_result.Length > 2 Then 'table.Rows.Count > 0 Then 'sqls.getDataSet("getidcompproduct") > 0 Then
    '        table = Newtonsoft.Json.JsonConvert.DeserializeObject(Of DataTable)(json_result)
    ...
    Private Sub btnCheckQuota_Click(sender As Object, e As EventArgs) Handles btnCheckQuota.Click
        ...
        If json_result.Length > 2 Then 'table.Rows.Count > 0 Then 'sqls.getDataSet("getusage") > 0 Then
            table = Newtonsoft.Json.JsonConvert.DeserializeObject(Of DataTable)(json_result)
    ...
    Private Sub btnHistory_Click(sender As Object, e As EventArgs) Handles btnHistory.Click
        Dim hp As New frmHistoryPayment(modCore.idcompany)
        hp.ShowDialog()
    End Sub
