Public Class frmNewPayment
    ...
    Private Sub Form_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If sqls.getDataSet("existhw") > 0 Then
            tmpidhardware = sqls.getDataSet("existhw", 0, "idhardware")
        Else
            sqls = New SQLs(mystring)
            sqls.DMLQuery("select convert(varchar,getdate(),105) as tanggal,convert(varchar,getdate(),108) as waktu", "content")
            If sqls.getDataSet("content") > 0 Then
            ...
            dtsqls.datasetSave("hardware", tmpidhardware, fl, vl, False)
        ...
        If tmpidcompanyproduct = -1 Then
            ...
            vcp.AddRange(New Object() {-1, tmpidcompany, tmpidproduct, tmpidhardware, IIf(tmpidcompanyrefferal = -1, DBNull.Value, tmpidcompanyrefferal), 0, 0, 0, "", tmpUserID, tmpnowTime})
            dtsqls.datasetSave("companyproduct", -1, fcp, vcp, False)
        ...
        colcaption.AddRange(New String() {"ID", "Content"})
        coltype.AddRange(New Type() {GetType(Long), GetType(String)})
        dspayment = sqls.dataSet

        luePaket.Properties.DataSource = dspayment.Tables("paymentoption")
    ...
    Private Function getidcompanyproduct(idcompany As Long, idproduct As Long, idhardware As Long) As Long
        ...
        If sqls.getDataSet("getidcompproduct") > 0 Then
            retval = CLng(sqls.getDataSet("getidcompproduct", 0, "idcompanyproduct"))
        ElseIf sqls.getDataSet("getidcompproduct") = 0 Then
            ...
            vs.AddRange(New Object() {-1, idcompany, idproduct, idhardware, IIf(tmpidcompanyrefferal = -1, DBNull.Value, tmpidcompanyrefferal), 0, 0, 0, "", tmpUserID, tmpnowTime})
            dtsqls.datasetSave("companyproduct", -1, fs, vs, False)
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        Dim idpaymentonline As Long = 0
        Dim idcompanyproduct As Long = getidcompanyproduct(tmpidcompany, tmpidproduct, tmpidhardware)
        Dim dtsqls As New dtsetSQLS(mystring)
        Dim fs As New List(Of String)
        Dim vs As New List(Of Object)
        ...
        vs.AddRange(New Object() {-1, lueRekening.EditValue, idcompanyproduct, luePaket.EditValue, tePaymentNo.Text, CDate(deTanggal.EditValue), tePaket.Tag, tePaket.Text, teBank.Text, teNorek.Text, teNama.Text, meRemarks.Text, 0, 0, "", tmpnowTime, tmpUserID})

        If dtsqls.datasetSave("payment", -1, fs, vs, False) = True Then
            sqls = New SQLs(mystring)
            sqls.DMLQuery("select idpayment from payment where paymentno='" & tePaymentNo.Text & "'", "getidpayment")


            If idpaymentonline = sqls.getDataSet("getidpayment", 0, "idpayment") Then
                idpaymentonline = sqls.getDataSet("getidpayment", 0, "idpayment")

                Dim flite As New List(Of String)
                Dim vlite As New List(Of Object)
                fs.AddRange(New String() {"idpayment", "idpaymentonline", "idbankoption", "idcompanyproduct", "idpaymentoption", "paymentno", "tanggalbayar", "nominalbayar", "nominaldata", "bankpengirim", "norekpengirim", "atasnamapengirim", "remarks", "isvalid", "isdeleted", "deletereason", "createddate", "createdby"})
                vs.AddRange(New Object() {-1, idpaymentonline, lueRekening.EditValue, IIf(idcompanyproduct = -1, DBNull.Value, idcompanyproduct), luePaket.EditValue, tePaymentNo.Text, CDate(deTanggal.EditValue), tePaket.Tag, tePaket.Text, teBank.Text, teNorek.Text, teNama.Text, meRemarks.Text, 0, 0, "", tmpnowTime, tmpUserID})
                Dim dtlite As New dtsetSQLI(dblite)
                dtlite.datasetSave("payment", -1, flite, vlite, False)
    ...
    Private Sub luePaket_EditValueChanged(sender As Object, e As EventArgs) Handles luePaket.EditValueChanged
        ...
        If mys.getDataSet("nominaldata") > 0 Then
            tePaket.Text = CLng(mys.getDataSet("nominaldata", 0, "nominaldata"))
            tePaket.Tag = CLng(mys.getDataSet("nominaldata", 0, "nominalbayar"))
        Else
            tePaket.Text = 0
            tePaket.Tag = 0
        End If
    ...
    Private Sub btnExistCheck_Click(sender As Object, e As EventArgs) Handles btnExistCheck.Click
        ...
        Dim idpaymentonline As Long = 0
        Dim idcompanyproduct As Long = getidcompanyproduct(tmpidcompany, tmpidproduct, tmpidhardware)
        Dim idproducttype As Long = getidproducttype(tmpidcompany, tmpidproduct, tmpidhardware)
        ...
        vs.AddRange(New String() {-1, vid, 1, idcompanyproduct, 0, tePaymentNo.Text, Format(tmpnowTime, "dd-MM-yyyy"), vnominal, vnominaldata, "Voucher", "-", "Voucher", "Voucher", 1, 0, "-", tmpUserID, tmpnowTime})
        ...
        If dtsqls.datasetSave("payment", -1, fs, vs, False) = True Then
            ...
            vlite.AddRange(New Object() {-1, idpaymentonline, vid, 1, idcompanyproduct, DBNull.Value, tePaymentNo.Text, tmpnowTime, vnominal, vnominaldata, teBank.Text, teNorek.Text, teNama.Text, meRemarks.Text, 1, 0, "", tmpnowTime, tmpUserID})
            Dim dtlite As New dtsetSQLI(dblite)
            dtlite.datasetSave("payment", -1, flite, vlite, False)
    ...
    Private Function getidproducttype(idcompany As Long, idproduct As Long, idhardware As Long) As Long
        Dim retval As Long = -1
        Dim sqls As New SQLs(mystring)
        sqls.DMLQuery("select p.idproducttype from companyproduct cp left join product p on cp.idproduct=p.idproduct where cp.idcompany=" & idcompany & " and cp.idproduct=" & idproduct & " and cp.idhardware=" & idhardware, "getprodtype")
        If sqls.getDataSet("getprodtype") > 0 Then
            retval = sqls.getDataSet("getprodtype", 0, "idproducttype")
        End If
        Return retval
    End Function
    ...
    Private Sub btnCheckQuota_Click(sender As Object, e As EventArgs) Handles btnCheckQuota.Click
        ...
        'sqls.DMLQuery("select (select isnull(sum(nominaldata),0) as nominaldata from payment where idcompanyproduct=" & idcompanyproduct & " and isvalid=1 and isdeleted=0)-(select isnull(sum(nominaldata),0) as nominaldata from datausage where idcompanyproduct=" & idcompanyproduct & " and isdeleted=0 and postdate=(select max(postdate) as postdate from datausage where idcompanyproduct=" & idcompanyproduct & " and isdeleted=0)) as nominaldata", "getkuota")
        ...
        End If
        'sqls.DMLQuery("select isnull(sum(nominaldata),0) as nominaldata from datausage where idcompanyproduct=" & idcompanyproduct & " and isdeleted=0 and postdate=(select max(postdate) as postdate from datausage where idcompanyproduct=" & idcompanyproduct & " and isdeleted=0)", "getusage")
        sqls.DMLQuery("select isnull(sum(nominaldata),0) as nominaldata from dailyusage where idcompanyproduct=" & idcompanyproduct & " and isdeleted=0 and postdate=(select max(postdate) as postdate from dailyusage where idcompanyproduct=" & idcompanyproduct & " and isdeleted=0)", "getusage")
